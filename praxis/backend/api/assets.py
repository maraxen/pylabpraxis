...
async def list_assets_by_type(asset_type: str, db_service: PraxisDBService = Depends(get_praxis_db_service)):
    """
    ...
    try:
        ...
        assets_data = await db_service.fetch_all_sql(query, params={"1": f"%{asset_type}%"})
...
async def list_available_assets_by_type(asset_type: str, db_service: PraxisDBService = Depends(get_praxis_db_service)):
    """
...
async def get_asset_details(asset_name: str, db_service: PraxisDBService = Depends(get_praxis_db_service)):
    """
...
@router.get("/search/{query_str}", response_model=List[AssetResponse]) # Renamed query to query_str to avoid conflict
async def search_assets(query_str: str, db_service: PraxisDBService = Depends(get_praxis_db_service)):
    """
    ...
    try:
        query = """
            ...
            ORDER BY name
        """
        assets_data = await db_service.fetch_all_sql(query, params={"1": f"%{query_str}%"})
        return [AssetResponse(**asset) for asset in assets_data]
    except Exception as e:
        raise HTTPException(
...
async def create_resource(request: ResourceCreationRequest, db_service: PraxisDBService = Depends(get_praxis_db_service)):
    try:
        resource_types = get_all_resource_types()
        resource_class = resource_types.get(request.resourceType)
        if not resource_class:
            raise HTTPException(status_code=400, detail=f"Unknown resource type: {request.resourceType}")

        valid_params = {}
        constructor_params_meta = get_resource_metadata().get(request.resourceType, {}).get("constructor_params", {})
        for param_name, param_info in constructor_params_meta.items():
            ...
            elif param_info.get("required"): # Pydantic models might not have "required" field, check .get
        ...
        plr_serialized_dict["type"] = request.resourceType # Ensure type is part of serialized data for consistency
...
async def create_machine(request: MachineCreationRequest, db_service: PraxisDBService = Depends(get_praxis_db_service)):
    try:
        machine_types = get_all_machine_types()
        machine_class = machine_types.get(request.machineType)
        if not machine_class:
            raise HTTPException(status_code=400, detail=f"Unknown machine type: {request.machineType}")

        valid_params = {}
        constructor_params_meta = get_machine_metadata().get(request.machineType, {}).get("constructor_params", {})
        for param_name, param_info in constructor_params_meta.items():
            if param_name in request.params:
                valid_params[param_name] = request.params[param_name]
        ...
        plr_serialized_dict["type"] = request.machineType # Ensure type is part of serialized data
...
async def get_labware_instance_inventory(instance_id: int, db: AsyncSession = Depends(get_db)) -> LabwareInventoryDataOut:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error parsing inventory data: {e}")

@router.put("/labware_instances/{instance_id}/inventory", response_model=LabwareInventoryDataOut, tags=["Labware Instances"])
async def update_labware_instance_inventory(instance_id: int, inventory_data: LabwareInventoryDataIn, db: AsyncSession = Depends(get_db)) -> LabwareInventoryDataOut:
    instance = await asset_data_service.get_labware_instance_by_id(db, instance_id)
    if not instance:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Labware instance not found")
    ...
    updated_instance = await asset_data_service.update_labware_instance_location_and_status(
        db,
...
async def create_labware_definition_endpoint(definition: LabwareDefinitionCreate, db: AsyncSession = Depends(get_db)):
    try:
        created_def = await asset_data_service.add_or_update_labware_definition(db=db, **definition.model_dump())
        return created_def
    except ValueError as e:
...
async def get_labware_definition_endpoint(pylabrobot_definition_name: str, db: AsyncSession = Depends(get_db)):
    db_def = await asset_data_service.get_labware_definition(db, pylabrobot_definition_name)
    if db_def is None:
...
async def list_labware_definitions_endpoint(db: AsyncSession = Depends(get_db), limit: int = 100, offset: int = 0):
    definitions = await asset_data_service.list_labware_definitions(db, limit=limit, offset=offset)
    return definitions
...
async def update_labware_definition_endpoint(
    ...
    existing_def = await asset_data_service.get_labware_definition(db, pylabrobot_definition_name)
    if not existing_def:
    ...
    if 'python_fqn' not in update_data and existing_def.python_fqn:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="python_fqn is required.")
...
async def delete_labware_definition_endpoint(pylabrobot_definition_name: str, db: AsyncSession = Depends(get_db)):
    success = await asset_data_service.delete_labware_definition(db, pylabrobot_definition_name)
    if not success:
...
async def create_deck_layout_endpoint(deck_layout_in: DeckLayoutCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        created_layout = await asset_data_service.create_deck_layout(
            db=db, layout_name=deck_layout_in.layout_name, deck_device_id=deck_layout_in.deck_device_id,
            description=deck_layout_in.description, slot_items_data=slot_items_data
        )
...
async def list_deck_layouts_endpoint(
    deck_device_id: Optional[int] = None, limit: int = 100, offset: int = 0, db: AsyncSession = Depends(get_db)
):
    layouts = await asset_data_service.list_deck_layouts(db, deck_device_id=deck_device_id, limit=limit, offset=offset)
    return layouts

@router.get("/deck_layouts/{deck_layout_id}", response_model=DeckLayoutResponse, tags=["Deck Layouts"])
async def get_deck_layout_by_id_endpoint(deck_layout_id: int, db: AsyncSession = Depends(get_db)):
...
async def update_deck_layout_endpoint(
    deck_layout_id: int, deck_layout_update: DeckLayoutUpdate, db: AsyncSession = Depends(get_db)
):
    ...
    try:
        updated_layout = await asset_data_service.update_deck_layout(
            db=db, deck_layout_id=deck_layout_id, name=deck_layout_update.layout_name,
            description=deck_layout_update.description, deck_device_id=deck_layout_update.deck_device_id,
...
async def delete_deck_layout_endpoint(deck_layout_id: int, db: AsyncSession = Depends(get_db)):
