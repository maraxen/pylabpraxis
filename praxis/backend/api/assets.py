...
async def list_assets_by_type(
  ...
  Examples of asset_type: 'resource', 'machine', 'resource'
  """
  try:
    query = """
            SELECT
...
async def create_machine(
  ...
  try:
    ...
    valid_params = {}
    constructor_params_meta = (
      get_machine_metadata().get(request.machineType, {}).get("constructor_params", {})
    )
    for param_name, param_info in constructor_params_meta.items():
      if param_name in request.params:
        valid_params[param_name] = request.params[param_name]
...
@router.get(
  "/resource_instances/{instance_id}/inventory",
  response_model=ResourceInventoryDataOut,
  tags=["Resource Instances"],
)
async def get_resource_instance_inventory(
  instance_id: int, db: AsyncSession = Depends(get_db)
) -> ResourceInventoryDataOut:
  instance = await asset_data_service.get_resource_instance_by_id(db, instance_id)
  if not instance:
    raise HTTPException(
...
async def update_resource_instance_inventory(
  instance_id: int,
  inventory_data: ResourceInventoryDataIn,
  db: AsyncSession = Depends(get_db),
) -> ResourceInventoryDataOut:
  instance = await svc.get_resource_instance_by_id(db, instance_id)
  if not instance:
    raise HTTPException(
      status_code=status.HTTP_404_NOT_FOUND, detail="Resource instance not found"
    )
  ...
  updated_instance = await svc.update_resource_instance_location_and_status(
    db,
  ...
  if not updated_instance or not updated_instance.properties_json:
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
      detail="Failed to update or retrieve resource instance properties",
    )
  try:
...
async def create_resource_definition_endpoint(
  ...
  try:
    created_def = await asset_data_service.add_or_update_resource_definition(
...
async def list_resource_definitions_endpoint(
  db: AsyncSession = Depends(get_db), limit: int = 100, offset: int = 0
):
  definitions = await svc.list_resource_definitions(db, limit=limit, offset=offset)
  return definitions
