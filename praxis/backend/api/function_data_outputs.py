...
async def list_function_data_outputs(
  filters: Annotated[FunctionDataOutputFilters, Depends()],
  db: Annotated[AsyncSession, Depends(get_db)],
...
async def read_well_data_outputs(
  filters: Annotated[WellDataOutputFilters, Depends()],
  db: Annotated[AsyncSession, Depends(get_db)],
...
async def read_plate_visualization_data(
  ...
  db: Annotated[AsyncSession, Depends(get_db)],
  data_type: Annotated[DataOutputTypeEnum | None, Query()] = None,
  function_call_id: Annotated[UUID | None, Query()] = None,
  protocol_run_id: Annotated[UUID | None, Query()] = None,
) -> PlateDataVisualization:
  ...
  result = await svc.read_plate_data_visualization(
    db=db,
    plate_resource_accession_id=plate_resource_id,
    data_type=data_type,
