...
from ..interfaces import WorkcellAssetsInterface # Keep for type hint if used by orchestrator
...
async def upload_config_file(file: UploadFile = File(...)):
    try:
        ...
        if file.filename is None: raise HTTPException(status_code=400, detail="No file selected")
        if not file.filename.endswith(".json"): raise HTTPException(status_code=400, detail="Invalid file format. JSON only.")
...
async def discover_protocols(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        definitions_orm: List[FunctionProtocolDefinitionOrm] = await protocol_data_service.list_protocol_definitions(
        ...
        for def_orm in definitions_orm:
            ...
            if def_orm.source_repository:
                path_to_report = f"git:{def_orm.source_repository.name}/{def_orm.source_file_path}"
            elif def_orm.file_system_source:
                 path_to_report = f"fs:{def_orm.file_system_source.name}/{def_orm.source_file_path}"
        ...
        raise HTTPException(status_code=500, detail=f"Failed to discover protocols: {str(e)}")
...
@router.get("/details", response_model=Dict) # Keep Dict for now, but could be a Pydantic model
async def get_protocol_details(
    protocol_definition_id: Optional[int] = None, # Added to fetch by ID
    protocol_path: Optional[str] = None, # Kept for compatibility if needed, but ID is preferred
    ...
    if not protocol_definition_id and not protocol_path:
        raise HTTPException(status_code=400, detail="protocol_definition_id or protocol_path is required.")

    try:
        def_orm: Optional[FunctionProtocolDefinitionOrm] = None
        if protocol_definition_id:
            def_orm = await protocol_data_service.get_protocol_definition_by_id(db, protocol_definition_id)
            if not def_orm:
                 raise HTTPException(status_code=404, detail=f"Protocol definition with ID {protocol_definition_id} not found.")
            protocol_path_for_module = def_orm.source_file_path # Needed for module import if still used
            ...
            # and extract, but this bypasses some DB benefits.
             logger.warning("Fetching protocol details by path is less reliable; prefer protocol_definition_id.")
             protocol_module = await import_protocol_module(protocol_path) # protocol_path here is the raw path
             if not protocol_module:
                 raise HTTPException(status_code=404, detail=f"Protocol module not found at path: {protocol_path}")
             details = await get_protocol_details_from_module(protocol_module, protocol_path) # protocol_path is raw
             return details # Bypasses DB Orm for details if only path given

        if not def_orm or not def_orm.source_file_path:
             raise HTTPException(status_code=404, detail="Protocol definition or its source path not found in DB.")
        ...
        details = {
            ...
            "parameters": {
                p.name: {
                    "type": _map_python_type_to_frontend(p.actual_type_str or p.type_hint_str or "unknown"),
...
async def get_protocol_details_from_module(
    protocol_module: Any, protocol_path: str # protocol_path is raw file path
) -> Dict:
    ...
    if not isinstance(baseline_parameters, dict): baseline_parameters = {}
    if not isinstance(required_assets, dict): required_assets = {}
...
def _map_python_type_to_frontend(python_type: str) -> str:
    ...
    mapping = {
        ...
        "bool": "boolean", "list": "array", "dict": "dict",
        "<class 'str'>": "string", "<class 'int'>": "integer",
        "<class 'float'>": "float", "<class 'bool'>": "boolean",
        "<class 'list'>": "array", "<class 'dict'>": "dict",
    }
...
async def import_protocol_module(protocol_raw_path: str) -> Optional[Any]:
    ...
    try:
        ...
        if not abs_path.exists():
            ...
            if not abs_path.exists():
                 logger.error(f"Protocol file also not found relative to project root: {abs_path}")
                 return None
        ...
        logger.error(f"Error importing protocol module from path '{protocol_raw_path}': {e}", exc_info=True)
        return None
...
async def get_protocol_schema(
    ...
    try:
        ...
        if protocol_definition_id:
            def_orm = await protocol_data_service.get_protocol_definition_by_id(db, protocol_definition_id)
            if not def_orm:
                raise HTTPException(status_code=404, detail=f"Protocol definition ID {protocol_definition_id} not found.")
            ...
            if def_orm.source_file_path:
                ...
                if protocol_module:
                    parameters_obj = getattr(protocol_module, "baseline_parameters", ProtocolParameters())
                else:
                    logger.warning(f"Could not load module for schema from {def_orm.source_file_path}")
            else:
                 logger.warning(f"No source_file_path for def ID {protocol_definition_id} to load module for schema.")


        elif protocol_path: # Legacy
            logger.warning("Fetching schema by path is less reliable; prefer protocol_definition_id.")
            protocol_module = await import_protocol_module(protocol_path)
            if not protocol_module:
                raise HTTPException(status_code=404, detail="Protocol module not found for schema generation.")
            parameters_obj = getattr(protocol_module, "baseline_parameters", ProtocolParameters())
        ...
        if not isinstance(parameters_obj, ProtocolParameters):
            ...
            # This case should be rare if loading from module
             logger.error(f"Loaded baseline_parameters is not a ProtocolParameters instance. Type: {type(parameters_obj)}")
             raise HTTPException(status_code=500, detail="Internal error: Could not load protocol parameters correctly for schema.")
        ...
        raise HTTPException(status_code=500, detail=f"Error generating protocol schema: {str(e)}")
...
async def prepare_protocol(
    ...
    try:
        ...
        possible_def = await db.execute(
            select(FunctionProtocolDefinitionOrm).filter(FunctionProtocolDefinitionOrm.source_file_path.like(f"%{Path(request.protocol_path).name}"))
        )
        def_orm_candidate = possible_def.scalars().first()

        if def_orm_candidate and def_orm_candidate.source_file_path: # Check if path matches somewhat
            # Use the get_protocol_details logic that constructs from ORM
            temp_details = await get_protocol_details(protocol_definition_id=def_orm_candidate.id, db=db)
            if temp_details.get("path") == request.protocol_path or Path(temp_details.get("path","")).name == Path(request.protocol_path).name:
                 details = temp_details
                 def_orm = def_orm_candidate # Found a match

        if not details: # Fallback to raw path loading
            logger.warning(f"Could not find exact DB match for path {request.protocol_path}, falling back to module loading for details.")
            protocol_module = await import_protocol_module(request.protocol_path)
            if not protocol_module:
                raise HTTPException(status_code=404, detail=f"Protocol module not found at path: {request.protocol_path}")
            details = await get_protocol_details_from_module(protocol_module, request.protocol_path)

        if not details: # Should not happen if module loading works
            raise HTTPException(status_code=404, detail="Protocol details could not be determined.")
                ...
                if param_name in parameters:
                    ...
                    if param_type_frontend == "number" or param_type_frontend == "integer":
                        try:
                            ...
                            if param_config.get("constraints", {}).get("integer_only") or param_type_frontend == "integer":
                                value = round(value)
        ...
        if required_assets_list:
            if request.asset_assignments: # Keys are asset requirement names, values are asset instance names/IDs
                # Verify assigned assets exist and are suitable (simplified here)
                for req_asset_name, assigned_asset_identifier in request.asset_assignments.items():
                    ...
                    asset_data = await db_service.fetch_one_sql(asset_query, params={"1": assigned_asset_identifier})
                    if not asset_data:
                        raise HTTPException(status_code=400, detail=f"Assigned asset '{assigned_asset_identifier}' for requirement '{req_asset_name}' not found.")
        ...
        config_to_validate = {
            ...
            "name": details["name"],
            "parameters": validated_params,
            "required_assets": request.asset_assignments or {}, # Asset requirement name -> instance name
        }

        validation = await orchestrator.validate_configuration(config_to_validate)
        if not validation["valid"]:
            return {"status": "invalid", "errors": validation["errors"], "config": config_to_validate}
...
async def start_protocol(
    config_payload: Dict[str, Any], # Changed from 'config' to avoid conflict with 'config' module
    orchestrator: Orchestrator = Depends(get_orchestrator)
):
    try:
        ...
        return ProtocolStatus(name=protocol.name, status=str(protocol.status)) # Ensure status is string
    except Exception as e:
...
async def send_control_command_to_run(run_guid: str, command: str):
    from praxis.backend.utils.run_control import send_control_command as send_control_command_to_redis, ALLOWED_COMMANDS
    ...
    try:
        ...
        if success: return {"message": f"Command '{cmd_upper}' sent."}
        else: raise HTTPException(status_code=500, detail="Failed to send command via Redis.")
    except Exception as e:
