...
async def list_resource_instances(
  ...
  Args:
      ...
      on_deck_slot (Optional[str], optional): Filter instances by the name of
          the deck slot they are currently in. Defaults to None.
      limit (int): The maximum number of results to return. Defaults to 100.
      offset (int): The number of results to skip before returning. Defaults to 0.
  ...
  logger.info(
    ...
    "machine_id=%s, deck_slot='%s', limit=%d, offset=%d.",
    pylabrobot_definition_name,
    status,
    location_machine_id,
    on_deck_slot,
  ...
  if on_deck_slot:
    ...
    logger.debug("Filtering by deck slot: '%s'.", on_deck_slot)
...
async def update_resource_instance_location_and_status(
  ...
  Args:
      ...
      current_deck_slot_name (Optional[str], optional): The name of the deck
          slot where the resource is now located. Defaults to None.
  ...
  if instance_orm:
    ...
    if (
      ...
      logger.debug(
        "Instance ID %d location update: machine_id=%s, deck_slot='%s', "
        "physical_desc='%s'.",
        resource_instance_id,
        location_machine_id,
        current_deck_slot_name,
