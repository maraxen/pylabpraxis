...
class WellDataOutputCRUDService(
    ...
    async def create(
        self, db: AsyncSession, *, obj_in: WellDataOutputCreate
    ) -> WellDataOutputOrm:
        """Create a single well data output."""
        log_prefix = (
            f"Well Data (Plate: {obj_in.plate_resource_accession_id}, "
            f"Well: {obj_in.well_name}):"
        )
        ...
        num_cols = (
            plate_dimensions["columns"] if plate_dimensions else 12
        )  # Default fallback
    ...
    async def get_multi(
        ...
        *,
        ...
        logger.info(
            "%s Found %d well data outputs.", log_prefix, len(well_data_list)
        )
...
async def create_well_data_outputs_from_flat_array(
    ...
    if not plate_dimensions:
        error_msg = (
            f"Could not determine plate dimensions for resource {plate_resource_accession_id}"
        )
