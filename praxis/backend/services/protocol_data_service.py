...
async def update_protocol_run_status( # MODIFIED: async def
    ...
    if db_protocol_run:
        ...
        utc_now = datetime.datetime.now(datetime.timezone.utc)
        if new_status == ProtocolRunStatusEnum.RUNNING and \
          not db_protocol_run.start_time:
          ...
          ProtocolRunStatusEnum.CANCELLED
            ...
            if db_protocol_run.start_time is not None and db_protocol_run.end_time is not None:
              db_protocol_run.duration_ms = int(
                (db_protocol_run.end_time - db_protocol_run.start_time).total_seconds() * 1000) # type: ignore
