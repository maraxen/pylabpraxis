...
class ScheduleEntryCRUDService(
    ...
    async def create(
        self, db: AsyncSession, *, obj_in: ScheduleEntryCreate
    ) -> ScheduleEntryOrm:
    ...
    async def get_multi(
        ...
        *,
        ...
        stmt = apply_date_range_filters(
            stmt, filters, cast(InstrumentedAttribute, self.model.created_at)
        )
...
async def get_scheduling_metrics(
    ...
    avg_stmt = select(
        func.avg(ScheduleHistoryOrm.id).label("avg_duration"),
    ).filter(
        and_(
            ScheduleHistoryOrm.created_at >= start_time,
            ScheduleHistoryOrm.created_at <= end_time,
