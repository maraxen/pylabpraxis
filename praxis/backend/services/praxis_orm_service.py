...
class PraxisDBService:
    ...
    async def initialize(
        ...
        if keycloak_dsn:
            ...
            while retries < cls._max_retries:
                try:
                    if not cls._keycloak_pool or cls._keycloak_pool._closed:  # type: ignore
                        logger.info(
                            f"Attempting to connect to Keycloak database: {keycloak_dsn.split('@')[-1]}"
                        )
                    ...
                    retries += 1
                    logger.warning(
                        f"Keycloak database connection attempt {retries} failed: {str(e)}"
                    )
                    if retries < cls._max_retries:
                        logger.info(
                            f"Retrying Keycloak connection in {current_retry_delay} seconds..."
                        )
                        await asyncio.sleep(current_retry_delay)
                        current_retry_delay *= 2
                    else:
                        logger.error(
                            f"Failed to connect to Keycloak database after {cls._max_retries} attempts."
                        )
                    ...
                    logger.error(
                        f"Unexpected error during Keycloak database initialization: {str(e)}"
                    )
                    raise
            ...
            logger.info(
                "Keycloak DSN not provided; Keycloak database pool will not be initialized by PraxisDBService."
            )
    ...
    async def get_keycloak_connection(self) -> AsyncIterator[asyncpg.Connection[Any]]:  # type: ignore
        ...
        try:
            ...
            if not isinstance(conn, asyncpg.Connection):  # type: ignore
                ...
                raise ConnectionError(
                    "Failed to acquire Keycloak connection from pool."
                )
    ...
    async def register_protocol_run(
        ...
        async with self.get_praxis_session() as session:
            ...
            if new_run.id is None:
                raise ValueError(
                    "Failed to create protocol run: no ID returned after flush/refresh."
                )

            logger.info(
                f"Registered new protocol run (ID: {new_run.id}, GUID: {new_run.run_guid})"
            )
            ...
            if run_orm:
                ...
                if user_data:  # Assuming UserOrm is a dict-like structure or has .get()
                    user_info = {
                        ...
                        else getattr(user_data, "id", None),
                        "username": user_data.get("username")
                        if isinstance(user_data, dict)
                        else getattr(user_data, "username", None),
                        "email": user_data.get("email")
                        if isinstance(user_data, dict)
                        else getattr(user_data, "email", None),
                        "first_name": user_data.get("first_name")
                        if isinstance(user_data, dict)
                        else getattr(user_data, "first_name", None),
                        "last_name": user_data.get("last_name")
                        if isinstance(user_data, dict)
                        else getattr(user_data, "last_name", None),
                    }
                ...
                return {
                    ...
                    "start_time": run_orm.start_time.isoformat()
                    if run_orm.start_time
                    else None,
                    "end_time": run_orm.end_time.isoformat()
                    if run_orm.end_time
                    else None,
    ...
    async def update_protocol_run_status(
        ...
        async with self.get_praxis_session() as session:
            ...
            logger.info(
                f"Protocol run {protocol_run_id} status updated to {status.name}."
            )
    ...
    async def list_protocol_runs(
        ...
        async with self.get_praxis_session() as session:
            ...
            if created_by_user_id:
                ...
                stmt = stmt.where(
                    ProtocolRunOrm.created_by_user.has(id=created_by_user_id)
                )  # type: ignore
        ...
        async with self.get_praxis_session() as session:
            ...
            if asset_orm:
                ...
                asset_orm.properties_json = (
                    ...
                    else asset_orm.properties_json
                )
            ...
            if asset_orm.id is None:
                raise ValueError(
                    f"Failed to add/update labware instance '{user_assigned_name}': no ID returned."
                )
            return asset_orm.id
        ...
        async with self.get_praxis_session() as session:
            ...
            if asset_orm:
                return {
                    ...
                    "expiry_date": asset_orm.expiry_date.isoformat()
                    if asset_orm.expiry_date
                    else None,
                    "date_added_to_inventory": asset_orm.date_added_to_inventory.isoformat()
                    if asset_orm.date_added_to_inventory
                    else None,
                    "current_status": asset_orm.current_status.name
                    if asset_orm.current_status
                    else None,
    ...
    async def fetch_one_sql(
        self, sql_query: str, params: Optional[dict] = None
    ) -> Optional[Dict[Any, Any]]:  # type: ignore
        async with self.get_praxis_session() as session:
            from sqlalchemy.sql import text
...
def _get_keycloak_dsn_from_config() -> Optional[str]:
    ...
    if not CONFIG_FILE_PATH.exists():
        logger.warning(
            f"praxis.ini not found at {CONFIG_FILE_PATH} for Keycloak DSN lookup."
        )
        return None
    ...
    if config.has_section("keycloak_database"):
        try:
            user = os.getenv(
                "KEYCLOAK_DB_USER", config.get("keycloak_database", "user")
            )
            password = os.getenv(
                "KEYCLOAK_DB_PASSWORD", config.get("keycloak_database", "password")
            )
            host = os.getenv(
                "KEYCLOAK_DB_HOST", config.get("keycloak_database", "host")
            )
            port = os.getenv(
                "KEYCLOAK_DB_PORT", config.get("keycloak_database", "port")
            )
            dbname = os.getenv(
                "KEYCLOAK_DB_NAME", config.get("keycloak_database", "dbname")
            )
