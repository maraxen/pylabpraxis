...
class DeckService(CRUDBase[DeckOrm, DeckCreate, DeckUpdate]):
    ...
    def _filter_and_convert_enums(self, deck_data: dict) -> dict:
        ...
        filtered_data = {
            key: value for key, value in deck_data.items() if key in valid_params
        }

        for attr_name, column in sa_inspect(self.model).columns.items():
            if (
                attr_name in filtered_data
                and hasattr(column.type, "enum_class")
            ):
    ...
    async def _handle_relationships(
        self, db: AsyncSession, deck_orm: DeckOrm, filtered_data: dict,
    ) -> None:
        ...
        if "parent_machine_accession_id" in filtered_data:
            machine = await db.get(
                MachineOrm, filtered_data["parent_machine_accession_id"],
            )
            if machine:
    ...
    async def create(
        self, db: AsyncSession, *, obj_in: DeckCreate,
    ) -> DeckOrm:
        """Create a new deck."""
        logger.info(
    ...
    async def get(
        self, db: AsyncSession, accession_id: str | UUID,
    ) -> DeckOrm | None:
    ...
    async def update(
        ...
        *,
        ...
        logger.info(
            "Attempting to update deck with ID: %s.", db_obj.accession_id,
        )
        ...
        await db.flush()
        await db.refresh(
            db_obj, ["parent", "parent_machine", "children", "deck_type"],
        )
        logger.info(
    ...
    async def remove(
        ...
        if not deck_orm:
            logger.warning(
                "Deck with ID %s not found for deletion.", accession_id,
            )
            return None
