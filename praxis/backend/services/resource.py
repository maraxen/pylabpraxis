...
class ResourceService(CRUDBase[ResourceOrm, ResourceCreate, ResourceUpdate]):
    ...
    async def create(
        ...
        *,
        ...
        await db.refresh(
            resource_orm, ["children", "parent", "resource_definition"],
        )
        logger.info(
    ...
    async def get_multi(
        ...
        *,
        ...
        if filters.parent_accession_id is not None:
            conditions.append(
                self.model.parent_accession_id
                == filters.parent_accession_id,
            )
    ...
    async def update(
        ...
        *,
        ...
        logger.info(
            "Attempting to update resource with ID: %s.", db_obj.accession_id,
        )

        obj_in_model = (
            ResourceUpdate(**obj_in)
        ...
        await db.flush()
        await db.refresh(
            db_obj, ["children", "parent", "resource_definition"],
        )
        logger.info(
    ...
    async def remove(
        ...
        if not resource_orm:
            logger.warning(
                "Resource with ID %s not found for deletion.", accession_id,
            )
            return None
    ...
    async def update_resource_location_and_status(
        ...
        *,
        ...
        return await self.update(
            db, db_obj=resource, obj_in=ResourceUpdate(**update_data),
        )
