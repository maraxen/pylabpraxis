...
class Orchestrator:
    ...
    async def _get_protocol_definition_orm_from_db(
        ...
        return await self.protocol_definition_service.get_by_name_and_version(
            db=db_session,
            name=protocol_name,
            version=version,
    ...
    async def _handle_pre_execution_checks(
        ...
        if command == "PAUSE":
            ...
            await self.protocol_run_service.update(
                ...
                obj_in={"status": ProtocolRunStatusEnum.PAUSED},
            )
            ...
            while True:
                ...
                if new_command == "RESUME":
                    ...
                    await self.protocol_run_service.update(
                        db_session,
                        db_obj=protocol_run_orm,
                        obj_in={"status": ProtocolRunStatusEnum.RUNNING},
                    )
                    await db_session.commit()
                    break
                if new_command == "CANCEL":
                    ...
                    await self.protocol_run_service.update(
                        ...
                        db_obj=protocol_run_orm,
                        obj_in={
                            "status": ProtocolRunStatusEnum.CANCELLED,
    ...
    async def _execute_protocol_main_logic(
        ...
        return result, acquired_assets_info
    ...
    def _inject_praxis_state(
        ...
        if protocol_pydantic_def.state_param_name:
            ...
            if state_param_meta:
                if "PraxisState" in state_param_meta.type_hint:
                ...
                elif "dict" in state_param_meta.type_hint.lower():
    ...
    async def _handle_deck_preconfiguration(
        ...
        if (
            protocol_pydantic_def.preconfigure_deck
            and protocol_pydantic_def.deck_param_name
        ):
            ...
            if deck_accession_identifier_from_user is not None:
                ...
                if isinstance(deck_accession_identifier_from_user, str):
                    deck_config_orm = (
                        await self.deck_service.get_by_name(
                    ...
                    deck_config_orm_accession_id_to_apply = (
                        deck_accession_identifier_from_user
                    )
    ...
    async def _prepare_arguments(
        ...
        self._process_input_parameters(
            protocol_pydantic_def, input_parameters, final_args
        )
        state_dict_to_pass = self._inject_praxis_state(
            protocol_pydantic_def, praxis_state, final_args
        )
    ...
    async def _handle_protocol_execution_error(
        ...
        try:
            last_good_snapshot = praxis_state.get(
                "workcell_last_successful_snapshot"
            )
    ...
    async def execute_protocol(
        ...
        async with self.db_session_factory() as db_session:
            ...
            protocol_run_db_obj = await self.protocol_run_service.create(
                db=db_session,
                obj_in={
            ...
            try:
                ...
                try:
                    await db_session.commit()
                    logger.info(
                        "ORCH: Final DB commit for run %s successful.", run_accession_id
                    )
        ...
        async with self.db_session_factory() as db_session:
            ...
            try:
                ...
                final_run_orm = await db_session.get(
                    ProtocolRunOrm, run_accession_id
                )
                if final_run_orm:
                    await self._finalize_protocol_run(
                        final_run_orm,
