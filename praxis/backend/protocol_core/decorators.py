...
def protocol_function(
    ...
    def decorator(func: Callable):
        ...
        if not resolved_name:
            raise ValueError("Protocol function name cannot be empty \
              (either provide 'name' argument or use a named function).")
        if is_top_level and top_level_name_format and \
          not re.match(top_level_name_format, resolved_name):
            raise ValueError(f"Top-level protocol name '{resolved_name}' does not match format: \
              {top_level_name_format}")
        ...
        def wrapper(*args, **kwargs):
            ...
            if not isinstance(praxis_run_context, PraxisRunContext):
                print(f"WARNING: Protocol '{current_meta['name']}' \
                  v{current_meta['version']}' called outside orchestrated context.")
            ...
            if praxis_run_context.current_db_session and this_function_def_db_id is not None:
                try:
                    ...
                    print(f"ERROR: Failed to log_function_call_start for '{current_meta['name']}': \
                      {log_e}")
            else:
                print(f"[ConsoleLog] START Call: {current_meta['name']} v{current_meta['version']} \
                  (DefID: {this_function_def_db_id}, Run: {praxis_run_context.run_guid}, ParentLog \
                    :{parent_log_id_for_this_call}) Args: Shortened for brevity")
            ...
            while True:
                ...
                if command not in ALLOWED_COMMANDS:
                    print(f"Warning: Invalid control command '{command}' received for run \
                      {run_guid}. Clearing.")
