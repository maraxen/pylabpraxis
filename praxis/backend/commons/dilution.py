...
async def dilution_checks(n_dilutions: int,
                          ...
                          dilution_axis: Optional[Literal["row",
                                                          "column",
                                                          "x",
                                                          "y",
                                                          0,
...
async def find_optimal_dilution_strategy(n_variables: int, # TODO: implement replicate handling
                                          n_dilutions: int,
  ...
  if variable_ids is None:
    variable_ids = [str(i) for i in range(n_variables)]
  variable_details: dict[str, dict] = {var: {"id": var} for var in variable_ids}
  if strategy == "all_row":
...
async def optimal_dilution_transfer(liquid_handler: LiquidHandler,
                                    source_plate: Plate,
                                    target_plate: Plate,
                                    dilution_factor: float,
                                    source_wells: list[int],
...
@liquid_handler_setup_check
async def antigen_dilution_series(liquid_handler: LiquidHandler,
  ...
  antigen_volumes, dilution_factors = await coerce_to_list([antigen_volumes, dilution_factors])
  ...
  antigen_volumes, dilution_factors = await check_list_length(
    ...
    coerce_length = True,
    target_length = len(antigen_ids))
  if dilution_axis == "optimal":
    antigen_dilution_details = await find_optimal_dilution_strategy(n_variables = len(antigen_ids),
  ...
  buffer_transfer_volumes = [antigen_volumes[i] - antigen_volumes[i]/dilution_factors[i] \
  ...
  if len(set(buffer_transfer_volumes)) == 1:
    if use_96:
      ...
      await transfer_buffer96(liquid_handler = liquid_handler,
    ...
    raise ValueError("Buffer transfer volumes not equal but 96 transfer head was specified \
      for buffer transfer")
  else:
    raise NotImplementedError("More complex dilution factors per variable not implemented")



@liquid_handler_setup_check
async def transfer_buffer96(liquid_handler: LiquidHandler,
                            tip_rack: TipRack,
  ...
  await liquid_handler.aspirate96(resource = buffer_reservoir, volume = buffer_transfer_volume)
...
@liquid_handler_setup_check
async def serial_dilution(liquid_handler: LiquidHandler, # TODO: handle single row and snaked dilutions that are longer than a column
                          tips: TipRack,
                          plate: Plate,
  ...
  dilution_factors, source_volumes = await coerce_to_list([dilution_factors, source_volumes])
  assert isinstance(dilution_factors, list) and isinstance(source_volumes, list)
  dilution_factors, source_volumes = await check_list_length([dilution_factors, source_volumes],
                                                             True,
                                                             len(variable_dict))
  ...
  for i in range(n_dilutions - 1):
    ...
    wells_from = [variable_dict[var]["initial_well_index"] + column_from \
      for var in single_row]
    wells_to = [variable_dict[var]["initial_well_index"] + column_to \
      for var in single_row]
...
async def handle_dilution(liquid_handler: LiquidHandler,
                          antigen_ids: list[str],
                          antigen_volumes: float | list[float],
                          n_replicates: int,
                          n_dilutions: int,
                          ...
                          buffer_tips: TipRack,
                          dilution_tips: TipRack,
                          constant_dilution: bool = True) -> None:
  """
  ...
  for i, antigen_id in enumerate(antigen_ids):
      ...
      await liquid_handler.pick_up_tips(tip_spots=buffer_tips[column_index:column_index + n_dilutions])
      await transfer_buffer(liquid_handler = liquid_handler,
                            buffer_reservoir = buffer_reservoir,
                            target_wells = dilution_wells,
                            target_plate = dilution_plate,
                            buffer_volumes = buffer_transfer_volumes,
                            buffer_total_volume = buffer_total_volume)
      await liquid_handler.drop_tips(tip_spots=buffer_tips[column_index:column_index + n_dilutions])
  await dilution_series(liquid_handler = liquid_handler,
                          tips = dilution_tips,
                          n_dilutions = n_dilutions,
                          dilution_factors = dilution_factors,
                          source_plate = dilution_plate,
...
async def transfer_buffer(liquid_handler: LiquidHandler,
                          buffer_reservoir: Plate,  # TODO: allow for reservoir
                          target_plate: Plate,
  ...
  if single_buffer:
    if isinstance(buffer_total_volume, list):
      if len(buffer_total_volume) > 1:
        raise ValueError("Total buffer volume variable list, but single buffer specified")
    if sum(buffer_transfer_volumes) > buffer_total_volume:
      raise Value("Buffer volume insufficient")
  await liquid_handler.aspirate(resources = buffer_reservoir[buffer_wells],
                                vols = buffer_volumes)
  await liquid_handler.dispense(target_plate[target_wells],
                                vols = buffer_volumes)

@liquid_handler_setup_check
async def transfer_top_row(liquid_handler: LiquidHandler,
                            tips: TipRack,
...
async def dilution_series(liquid_handler: LiquidHandler,
                          tips: TipRack,
                          n_dilutions: int,
                          dilution_factors: float | list[float],
                          source_plate:Plate,  # TODO: allow for reservoir
  ...
  if n_dilutions > source_plate.num_items_y + 1:
    raise ExperimentError("Number of dilutions exceeds number of wells per column in plate.")
  if n_replicates > source_plate.num_items_x:
    raise ExperimentError("Number of replicates exceeds number of wells per row in plate.")
  if not target_plate or target_plate == source_plate:
    target_plate = source_plate
  else:
    await transfer_top_row(liquid_handler = liquid_handler,
                      tips=tips,
                      source_plate = source_plate,
                      target_plate = target_plate)
  for i in range(n_replicates):
    ...
    for j in range(n_dilutions):
      ...
      if j != len(range(n_dilutions)):
        await liquid_handler.dispense(target_plate[well_number + 1], vols = [transfer_volume])
    await liquid_handler.dispense(target_plate[well_number + 1],
                                  vols = [0],
                                  mix_volume = [transfer_volume * 10],
                                  mix_cycles = [mix_cycles])
