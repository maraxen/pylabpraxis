...
def get_class_members(library_name: str, subpackage, base_class_name: str):
    ...
    all_members = {}
    base_class = getattr(importlib.import_module(name=library_name + "." + subpackage),
                          base_class_name)
...
def patch_subclasses() -> dict[str, Type[Machine]]:
  ...
  """
  new_objs = {}
  for name, obj in get_class_members(library_name="pylabrobot",
                                    subpackage="machines",
                                    base_class_name=Machine.__name__).items():
