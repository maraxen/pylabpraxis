...
try:
  ...
  logger.info(
    f"Praxis Database URL configured: {praxis_database_url.replace(praxis_db_password, '****')}"
  )
  ...
  logger.warning(
    f"Falling back to default praxis_database_url: {praxis_database_url.replace('password', '****')}"
  )
...
try:
  if config_parser.has_section("keycloak_database"):
    ...
    logger.info(
      f"Keycloak DSN successfully parsed from config for PraxisDBService: {keycloak_dsn_from_config.replace(keycloak_db_password, '****')}"
    )
  else:
    logger.info(
      "No [keycloak_database] section in praxis.ini; Keycloak DSN not available from this module for PraxisDBService."
    )
except Exception as e:
...
async def init_praxis_db_schema():
  ...
  logger.info(
    f"Initializing Praxis database tables at {PRAXIS_DATABASE_URL.replace(praxis_db_password, '****')}..."
  )
  try:
