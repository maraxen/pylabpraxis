...
class ResourceDefinitionBase(PraxisBase):
    ...
    num_items: int | None = Field(default=None, index=True, description="Number of items (wells, tips)")
    plate_type: str | None = Field(default=None, index=True, description="Plate skirt type")
    well_volume_ul: float | None = Field(default=None, index=True, description="Well volume for plates")
    tip_volume_ul: float | None = Field(default=None, index=True, description="Tip volume for tip racks")
...
class ResourceBase(AssetBase):
    ...
    status: ResourceStatusEnum = Field(
        ...
        sa_column=Column(SAEnum(ResourceStatusEnum, name="resource_status_enum"), nullable=False, index=True),
    )
