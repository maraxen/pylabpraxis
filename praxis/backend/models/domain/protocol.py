...
class ProtocolRunBase(PraxisBase):
    ...
    function_file: str | None = Field(default=None, description="File containing the protocol function")
    status: ProtocolRunStatusEnum = Field(
        default=ProtocolRunStatusEnum.QUEUED,
        sa_column=Column(SAEnum(ProtocolRunStatusEnum, name="protocol_run_status_enum"), nullable=False, index=True),
    )
    started_at: datetime | None = Field(default=None, index=True, description="When the protocol run started")
    completed_at: datetime | None = Field(default=None, index=True, description="When the protocol run completed")
