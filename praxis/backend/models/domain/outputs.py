...
class FunctionDataOutput(FunctionDataOutputBase, table=True):
    ...
    data_json: dict[str, Any] | None = json_field(
        default=None, description="Data output as JSON"
    )
...
class WellDataOutputBase(PraxisBase):
    ...
    well_position: str | None = Field(default=None, index=True, description="Well position (e.g., 'A1')")
