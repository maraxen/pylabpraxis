...
class FunctionDataOutputBase(PraxisBase):
  ...
  data_type: DataOutputTypeEnum = Field(description="Type of data output", index=True)
  data_key: str = Field(
    max_length=255, description="Unique key within the function call", index=True
  )
  spatial_context: SpatialContextEnum = Field(
    default=SpatialContextEnum.UNKNOWN, description="Spatial context of the data"
  )
  ...
  data_units: str | None = Field(
    default=None, max_length=50, description="Units of measurement"
  )
...
class FunctionDataOutput(FunctionDataOutputBase, table=True):
  ...
  data_json: dict[str, Any] | None = Field(
...
class PlateDataVisualization(PraxisBase):
  ...
  plate_layout: dict[str, Any] = Field(..., sa_type=JsonVariant, description="Plate layout information")
...
class FunctionDataOutputFilters(SQLModel):
  ...
  spatial_contexts: list[SpatialContextEnum] | None = Field(None, description="Filter by spatial context.")
