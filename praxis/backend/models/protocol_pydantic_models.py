...
class ProtocolStartRequest(BaseModel):
    ...
    deck_configuration: Optional[str] = None  # This could be a DeckConfigurationOrm ID or name
    workcell_id: Optional[int] = None  # ID of the WorkcellOrm, if applicable
    protocol_definition_id: Optional[int] = None  # ID of the FunctionProtocolDefinitionOrm, if applicable
...
class ProtocolPrepareRequest(BaseModel):
    protocol_path: str # This will likely become a protocol_definition_id or name/version
...
class ParameterMetadataModel(BaseModel):
    ...
    constraints_json: ParameterConstraintsModel = Field(default_factory=ParameterConstraintsModel) # For things like min/max, length, etc. TODO: make ParameterConstraintsModel
    ui_hint: UIHint = Field(default_factory=UIHint) # For frontend rendering hints

class AssetConstraintsModel(BaseModel):
    required_methods: List[str] = Field(default_factory=list) # Methods that must be available on the asset
    required_attributes: List[str] = Field(default_factory=list) # Attributes that must be present on the asset
    required_method_signatures: Dict[str, str] = Field(default_factory=dict) # Method names and their expected signatures
    required_method_args: Dict[str, List[str]] = Field(default_factory=dict) # Method names and their required argument names, less strict than signature
...
class AssetRequirementModel(BaseModel):
    ...
    constraints: AssetConstraintsModel = Field(default_factory=AssetConstraintsModel) # ADDED FIELD
...
class ProtocolParameters(BaseModel):
    ...
    """
    user_parameters: Dict[str, ParameterMetadataModel] = Field(default_factory=dict)  # User-defined parameters
    system_parameters: Dict[str, ParameterMetadataModel] = Field(default_factory=dict)  # System-generated parameters (e.g., run GUID, timestamps)
