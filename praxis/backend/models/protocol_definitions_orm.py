...
from typing import (
    Optional,
)  # Any can be used for JSON if type is not strictly dict/list
import enum
...
class ProtocolRunOrm(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    run_guid: Mapped[str] = mapped_column(
    ...
    input_parameters_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
    resolved_assets_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
    output_data_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
    initial_state_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
    final_state_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
...
class FunctionCallLogOrm(Base):
    ...
    duration_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    input_args_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
    return_value_json: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Using dict
