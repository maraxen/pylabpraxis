...
class MachineDefinitionUpdate(BaseModel):
  ...
  machine_category: MachineCategoryEnum | None = Field(
    None,
    description="The new category of the machine.",
  )
  description: str | None = Field(
    None,
    description="The new description of the machine definition.",
  )
  is_consumable: bool | None = Field(None, description="Whether the machine is a consumable.")
  nominal_volume_ul: float | None = Field(
    None,
    description="The new nominal volume in microliters.",
  )
  ...
  plr_definition_details_json: dict[str, Any] | None = Field(
    None,
    description="New PyLabRobot specific definition details.",
  )
  size_x_mm: float | None = Field(
    None,
    description="The new size in the x-dimension in millimeters.",
  )
  size_y_mm: float | None = Field(
    None,
    description="The new size in the y-dimension in millimeters.",
  )
  size_z_mm: float | None = Field(
    None,
    description="The new size in the z-dimension in millimeters.",
  )
  ...
  resource_definition_accession_id: UUID7 | None = Field(
    None,
    description="The new accession ID of the resource definition.",
  )
  has_deck: bool | None = Field(None, description="Whether the machine has a deck.")
  deck_definition_accession_id: UUID7 | None = Field(
    None,
    description="The new accession ID of the deck definition.",
  )
  setup_method_json: dict[str, Any] | None = Field(
    None,
    description="The new setup method for the machine.",
  )
