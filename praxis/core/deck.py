...
class DeckManager:
    ...
    async def get_deck(self, deck_file: str) -> Deck:
        ...
        async with aiofiles.open(deck_path, mode="r") as f:
            deck_data = json.loads(await f.read())
            return Deck.deserialize(deck_data)

    async def get_available_deck_files(self) -> List[str]:
    ...
    async def get_baseline_deck(self, liquid_handler_name: str) -> Deck:
