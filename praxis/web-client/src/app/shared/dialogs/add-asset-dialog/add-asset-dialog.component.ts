import { Component, inject, signal, computed, ViewChild, OnInit, AfterViewInit, Input } from '@angular/core';
...
@Component({
    ...
    <div class="h-full flex flex-col max-h-[90vh] min-w-[700px]">
      ...
      <mat-dialog-actions align="end" class="px-6 py-4 border-t sys-border bg-sys-surface">
        <button mat-button (click)="goBack()" [disabled]="stepper.selectedIndex === 0">Back</button>
...
export class AddAssetDialogComponent implements OnInit, AfterViewInit {
    ...
    canGoForward(): boolean {
        if (this.stepper.selectedIndex === 0) return !!this.assetType();
        if (this.stepper.selectedIndex === 1) return !!this.selectedDefinition();
        return false;
    }
