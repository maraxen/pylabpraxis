...
void main() {
  ...
  group('AssetManagementBloc', () {
    ...
    final labwareDef1 = LabwareDefinitionCatalogOrm(id: 'lwDef1', name: 'Labware Def 1');
    final labwareInstance1 = LabwareInstanceOrm(id: 'lwInstance1', labwareDefinitionId: 'lwDef1');
    final deckLayout1 = DeckLayoutOrm(id: 'deck1', name: 'Deck 1');

    final List<ManagedDeviceOrm> mockDevices = [device1];
    final List<LabwareDefinitionCatalogOrm> mockLabwareDefinitions = [labwareDef1];
    ...
    test('initial state is AssetManagementInitial', () {
      expect(AssetManagementBloc(mockAssetApiService).state,
    ...
    group('AssetManagementLoadStarted', () {
      blocTest<AssetManagementBloc, AssetManagementState>(
        ...
        build: () {
          when(mockAssetApiService.getDevices())
              .thenAnswer((_) async => mockDevices);
          when(mockAssetApiService.getLabwareDefinitions())
              .thenAnswer((_) async => mockLabwareDefinitions);
          when(mockAssetApiService.getLabwareInstances())
              .thenAnswer((_) async => mockLabwareInstances);
          when(mockAssetApiService.getDeckLayouts())
      ...
      blocTest<AssetManagementBloc, AssetManagementState>(
        ...
        expect: () => [
          const AssetManagementLoadInProgress(),
          isA<AssetManagementLoadFailure>()
              .having((s) => s.error, 'error', 'Exception: Failed to fetch devices'),
        ],
        verify: (_) {
          verify(mockAssetApiService.getDevices()).called(1);
    ...
    group('AddDeviceStarted', () {
      ...
      blocTest<AssetManagementBloc, AssetManagementState>(
        ...
        build: () {
          when(mockAssetApiService.createDevice(any))
              .thenAnswer((_) async => newDevice);
          // For the subsequent load
          when(mockAssetApiService.getDevices()).thenAnswer((_) async => [newDevice]); // Assume it's now in the list
          when(mockAssetApiService.getLabwareDefinitions()).thenAnswer((_) async => mockLabwareDefinitions);
          when(mockAssetApiService.getLabwareInstances()).thenAnswer((_) async => mockLabwareInstances);
          when(mockAssetApiService.getDeckLayouts()).thenAnswer((_) async => mockDeckLayouts);
          return AssetManagementBloc(mockAssetApiService);
        },
      ...
      blocTest<AssetManagementBloc, AssetManagementState>(
        ...
        expect: () => [
          const AssetManagementUpdateInProgress(),
          isA<AssetManagementUpdateFailure>()
              .having((s) => s.error, 'error', 'Exception: Failed to create device'),
        ],
        verify: (_) {
          verify(mockAssetApiService.createDevice(newDevice)).called(1);
        },
