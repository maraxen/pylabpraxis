from praxis.protocol.standalone_task import StandaloneTask  # Assuming it's in the same protocol submodule
import datetime


class PlateReaderTask(StandaloneTask):
    def __init__(self, experiment_name, plate_name, measurement_type, wells, estimated_duration, registry, data_instance, config_file="config.ini"):
        super().__init__(experiment_name, estimated_duration, registry, data_instance, config_file)
    ...
    async def _setup(self):
        ...
        earliest_finish_time = datetime.datetime.now() + datetime.timedelta(seconds=self.estimated_duration)
        ...
        if not self.registry.acquire_lock("plate_reader", self.experiment_name, self.task_id, lock_timeout=self.estimated_duration, acquire_timeout=10):
            raise Exception("Failed to acquire lock on plate reader.")

    async def _execute(self):
        # Simulate performing plate reader operation
        print(f"Performing {self.measurement_type} on plate {self.plate_name} for wells {self.wells}.")
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (self.experiment_name, self.plate_name, self.measurement_type, json.dumps(self.wells), datetime.datetime.now()))
    ...
    async def _stop(self):
        ...
        self.registry.release_lock("plate_reader", self.experiment_name, self.task_id)
