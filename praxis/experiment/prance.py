...
from praxis.commons.commons import NamedPumpArray, SampleReservoir, TipWasher, StabilizedWells, PumpManager
...
needed_deck_resources = {"sample_reservoir": SampleReservoir,
                          ...
                          "bleach_water_tip_washer": TipWasher,
                          "water_only_tip_washer": TipWasher,
...
class Prance(Experiment):
  ...
  def __init__(self,
    ...
    self.cycle_start_time = datetime.datetime.now() if not self.already_ran else self["cycle_start_time"]
    self.reader_plate_stack_in_use = self["to_read_plate_stack"] if not self.already_ran else self["reader_plate_stack_in_use"]
  ...
  def generate_optimal_layout(self, n_rows: int = 8, n_columns: int = 6):
    """
    ...
    wells_per_bacteria = (n_rows * n_columns) // len(self["bacteria_ids"])
    n_phage_replicates = (wells_per_bacteria - self["no_phage_control"]) // \
      len(self["phage_ids"])
    idx = 0
    for i, bacteria_id in enumerate(self["bacteria_ids"]):
      for j in range(wells_per_bacteria):
        idx += 1
        #xy = (idx // n_rows, idx % n_rows)
        phage_idx = j // n_phage_replicates
        self[f"{bacteria_id}"].append(idx)
        if phage_idx >= len(self["phage_ids"]):
          self[f"phage_control"].append(idx)
        else:
          self[f"{self.parameters['phage_ids'][phage_idx]}"].append(idx)
    assert self.liquid_handler is not None, "Liquid handler is not defined."
    lagoon_plate: Plate = cast(Plate, self.liquid_handler.get_resource("lagoon_plate"))
    for well in get_all_wells(lagoon_plate):
      pass
  ...
  async def _execute(self):
    ...
    async def empty_tip_washer():
      ...
      await self.pump_manager.pump_volume(pump_name="auxiliary_pump",
                                                volume=self.parameters[f"washer_volume"] * 4,
                                                use_channels="waste",
                                                speed=0.8)
      washer_state = {"is_filled": False, "last_filled": datetime.datetime.now()}
      await self.update_resource_state([self.water_only_tip_washer, self.bleach_water_tip_washer],
                                  washer_state)
    ...
    async def handle_tip_washer():
      ...
      """
      tip_washers = [self.water_only_tip_washer, self.bleach_water_tip_washer]
      tip_washers_filled: list[bool] = \
        [self[f"{tip_washer.name}"]["is_filled"] for tip_washer in tip_washers]
      last_fills = \
        [self[f"{tip_washer.name}"]["last_filled"] for tip_washer in tip_washers]
      if not all(tip_washers_filled) and any(tip_washers_filled):
        raise ValueError("Invalid state. Tip washers are not all filled or not all empty.")
      if all(tip_washers_filled):
        if any(datetime.datetime.now() - last_fill >= self.parameters["basin_refill_frequency"] \
          for last_fill in last_fills):
        ...
        while self[self.sample_reservoir.name]["is_dirty"]:
          match self[self.sample_reservoir.name]["clean_progress"]:
            ...
            case "water_wash":
              ...
              await self.sync_state()
      self[self.sample_reservoir.name]["last_cleaned"] = datetime.datetime.now()
    ...
    async def transfer_and_bleach_tips(
      ...
      bleach_time = datetime.datetime.now()
      ...
      for well_set in well_sets:
        if not all(self[well.name]["transfer_substep"] for well in well_set):
          await self.update_resource_state(well_set, {"transfer_substep": "station_to_holding",
                                                    "last_bacteria_added": datetime.datetime.now(),
    ...
    async def transfer_samples():
      ...
      if ["last_bacteria_added"] not in self.runtime_state:
        self.runtime_state["last_bacteria_added"] = datetime.datetime.now()
        self.runtime_state["last_bacteria_added_id"] = self.parameters["bacteria_ids"][0]
        await self.sync_state()
    ...
    while True:
      match self.runtime_state["step"]:
        ...
        case "wait_for_next_cycle":
          if datetime.datetime.now() - self.cycle_start_time >= self.parameters["cycle_time"]:
            self["cycle_n"] += 1
  ...
  async def _setup(self):
    ...
    self.water_only_tip_washer: TipWasher = cast(TipWasher,
                                                  deck.get_resource("water_only_tip_washer"))
    self.bleach_water_tip_washer: TipWasher = cast(TipWasher,
    ...
    if self.parameters["prefilled_basins"]:
      for tip_washer in [self.water_only_tip_washer, self.bleach_water_tip_washer]:
        self.runtime_state[f"{tip_washer.name}"]["is_filled"] = True
        self.runtime_state[f"{tip_washer.name}"]["last_filled"] = self.start_time
    await self.sync_state()
