...
class Experiment(ABC):
    ...
    async def _check_experiment_configuration(
        ...
        """
        if not self.lab_configuration.has_all_specified_resources(
            experiment_configuration.resources
        ):
            raise ValueError(
                "Lab configuration does not have all the specified resources."
            )
        ...
        if experiment_configuration.directory != os.path.dirname(self.__file__):
            raise ValueError(
                "Experiment directory must be the same as the experiment file."
            )
        ...
        if experiment_configuration.data_directory not in os.path.dirname(
            self.__file__
        ):
    ...
    async def get_status(self):
        ...
        self.logger.info(
            "Experiment %s status queried; status: %s", self.name, self.status
        )
        self.plr_logger.info(
            "Experiment %s status queried; status: %s", self.name, self.status
        )
    ...
    async def _create_needed_dirs(
        self, directory_names: list[PathLike] = ("data")
        ...
        for directory_name in directory_names:
            if not os.path.exists(
                os.path.join(self.experiment_directory, directory_name)
            ):
