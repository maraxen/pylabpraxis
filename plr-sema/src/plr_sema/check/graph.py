...
reason (B4 -- see ``plr_sema.verdict.REASON_VOCABULARY``'s docstring: the
...
| ``method_name`` (paired with ``receiver_type``), checked against the contract table | reason ``unsupported_tool`` (redefined 260901 T11: key absent from the whole-survey contract table, not ``SUPPORTED_TOOLS`` membership -- see ``plr_sema.check``'s module docstring) |
