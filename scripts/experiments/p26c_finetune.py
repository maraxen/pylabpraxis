"""P2.6c: retrain the arm-A recipe on the assembly-0.1.5 corpus (natural lane now
covering the out-of-surface floor rows) and score it on the FROZEN 228-row eval
split against arm A2 as control (task 260903_p26c_oos_natural, backlog 4933,
prereg .praxia/docs/preregistration/260903_p26c-oos-natural-prereg.md).
...
trainer's ``--compare-report`` / ``--probe-pairs`` options (P2.6b plumbing; P2.6c adds tripwire3_recovered and the probe split).
...
Cluster invocation (first word = scripts/slurm/bth_run.sh, see lesson 469)::
    ...
    bth submit --remote engaging --preset gpu --name p26c-arm-A --no-push-first -- \
        ...
        bth run --script-path scripts/experiments/p26c_finetune.py \
            --output-paths outputs/p26c/A/result.json --tags arm:A3 -- \
            --arm A --seed 0 --eval-after --out-dir outputs/p26c/A \
            --compare-report training/eval/reports/260902_p26b_A.json \
...
_BOOTSTRAP_FLAG = "PRAXIS_P26C_BOOTSTRAPPED"
...
def _bootstrap_into_training_env() -> None:
    ...
    if os.environ.get(_BOOTSTRAP_FLAG):
        sys.stderr.write(
            "p26c_finetune: praxis_training still not importable after re-exec; "
    ...
    uv = shutil.which("uv")
    if uv is None:
        sys.stderr.write("p26c_finetune: neither .venv/bin/python nor uv found\n")
        raise SystemExit(3)
