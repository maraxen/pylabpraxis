"""P2.6d: retrain the arm-A recipe on the assembly-0.1.6 corpus (matrix v3: six
near-surface out-of-surface cells, 96 train rows + a 24-row near-surface probe)
and score it on the FROZEN 228-row eval split against arm A3 as control (task 260903_p26d_near_surface, backlog 4940,
prereg .praxia/docs/preregistration/260903_p26d-near-surface-prereg.md).
...
trainer's ``--compare-report`` / ``--probe-pairs`` options (P2.6b/P2.6c plumbing, unchanged: control = A3's report).
...
Cluster invocation (first word = scripts/slurm/bth_run.sh, see lesson 469)::
    ...
    bth submit --remote engaging --preset gpu --name p26d-arm-A --no-push-first -- \
        ...
        bth run --script-path scripts/experiments/p26d_finetune.py \
            --output-paths outputs/p26d/A/result.json --tags arm:A4 -- \
            --arm A --seed 0 --eval-after --out-dir outputs/p26d/A \
            --compare-report training/eval/reports/260903_p26c_A3.json \
...
_BOOTSTRAP_FLAG = "PRAXIS_P26D_BOOTSTRAPPED"
...
def _bootstrap_into_training_env() -> None:
    ...
    if os.environ.get(_BOOTSTRAP_FLAG):
        sys.stderr.write(
            "p26d_finetune: praxis_training still not importable after re-exec; "
    ...
    uv = shutil.which("uv")
    if uv is None:
        sys.stderr.write("p26d_finetune: neither .venv/bin/python nor uv found\n")
        raise SystemExit(3)
