"""Oracle spike: does PLR's simulator agree with plr-preflight's static verdict?

Feasibility probe for the oracle harness (plan
``.praxia/docs/plans/260902_plr-preflight-oracle-harness.md``). For each
...
* **static** -- the same call sequence adapted into the §6.2 graph wire
  ...
  ``plr_preflight.check_graph`` against the shipped derived-contract table.
...
Usage::
    ...
    uv run python plr-preflight/scripts/oracle_spike.py \
...
DEFAULT_CONTRACTS = REPO_ROOT / "plr-preflight" / "data" / "derived_contracts.json"
...
def run_static(graph: dict[str, Any], contracts_json: str) -> dict[str, dict[str, Any]]:
    ...
    from plr_preflight import check_graph
    from plr_preflight.verdict import join
