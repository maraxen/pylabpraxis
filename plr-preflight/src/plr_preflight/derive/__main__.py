"""``python -m plr_preflight.derive``: the derivation-pipeline CLI (spec 260901
§7.3/§7.4).
...
``Surface`` (``plr_preflight._provenance.Surface``) this run is against, recorded
...
from plr_preflight._provenance import DEFAULT_SURFACE, Surface, survey_stamp
...
from plr_preflight.derive import _stamp_to_dict  # noqa: SLF001 - same-package reuse
...
def build_derived_contracts_payload(
    ...
    **Population, not just the 1,314 finding-bearing methods (T11 item 4's
    ...
    ``plr_preflight.check``'s module docstring).
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m plr_preflight.derive", description=__doc__
    )
    parser.add_argument(
        "--survey-json",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "plr_preflight._provenance.DEFAULT_SURFACE's own name, not a second "
