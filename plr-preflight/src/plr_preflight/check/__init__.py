"""plr_preflight.check: the browser-side checker (spec 260901 §6).
...
``plr-preflight/data/derived_contracts.json`` (§7.3), the build artifact
``plr_preflight.derive`` regenerates from the survey.
...
3. Otherwise, the resolved ``DerivedContract`` (§7.2/§7.3) is walked:
   * every recorded gap (``unresolved_delegate`` / ``no_contract_derived``,
     the only two ``plr_preflight.derive`` ever emits, §7.2) becomes one Finding
   ...
   * every guard becomes one ``guard_predicate_unparsed`` Finding (§3.3: "a
     ...
     ``plr_preflight.verdict.REASON_VOCABULARY``'s docstring for the reinstatement
     path.)
...
(``plr_preflight.derive`` now derives a contract for every one of the whole
survey's 4,770 methods, not just the 10 -- see
``plr_preflight.derive.__main__.build_derived_contracts_payload``), so the two
...
``plr_preflight.verdict.join``'s docstring.
"""
...
from plr_preflight.check.graph import OperationNode, ProtocolComputationGraph, parse_graph
from plr_preflight.telemetry import emit_finding
from plr_preflight.verdict import AnalysisReport, Finding, PlrSite, Verdict, join
...
def _internal_error(op: OperationNode, *, detail: str = "") -> Finding:
    ...
    ``plr_preflight.derive`` ever emits (``no_contract_derived`` /
    ``unresolved_delegate``, §7.2) would itself be a drift between
    ``plr_preflight.derive`` and ``plr_preflight.check``, not a fact about PLR --
...
def _check(graph: ProtocolComputationGraph, contracts_payload: dict[str, Any]) -> AnalysisReport:
    ...
    # previously nothing under check/ ever called plr_preflight.telemetry.emit*,
...
def check_graph(graph_json: str, contracts_json: str) -> AnalysisReport:
    ...
    never shells out. Emits every finding via ``plr_preflight.telemetry`` (M2,
