"""plr_preflight.check.graph: stdlib-dataclass mirror of the extracted protocol
...
``src/plr_preflight/``) and unmovable per §1.1 (round 1 moves nothing out of
...
reason (B4 -- see ``plr_preflight.verdict.REASON_VOCABULARY``'s docstring: the
...
| ``method_name`` (paired with ``receiver_type``), checked against the contract table | reason ``unsupported_tool`` (redefined 260901 T11: key absent from the whole-survey contract table, not ``SUPPORTED_TOOLS`` membership -- see ``plr_preflight.check``'s module docstring) |
