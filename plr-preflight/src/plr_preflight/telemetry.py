"""plr_preflight.telemetry: emission surface for analyzer events (spec 260901 §4).
...
scale for a *dynamic* harness. plr-preflight is a *static* analyzer, so four of
...
| ``unsupported_tool``     | method outside ``SUPPORTED_TOOLS``  | method not resolvable to any entry in the whole-survey derived contract table (260901 T11 -- no longer ``SUPPORTED_TOOLS``, which is now the DYNAMIC harness's own boundary only; see ``plr_preflight.check`` module docstring) |
...
handed; callers are expected to pass ``plr_preflight._provenance.survey_stamp()``,
...
from plr_preflight.verdict import REASON_VOCABULARY, Finding, PlrSite, Verdict
