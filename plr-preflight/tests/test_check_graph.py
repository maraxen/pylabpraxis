"""Spec 260901 §6.3/§6.5, T8 (backlog #4834): `plr_preflight.check`'s round-1 entry
...
from plr_preflight.check import SUPPORTED_TOOLS, check_graph
from plr_preflight.verdict import AnalysisReport, Finding, PlrSite, Verdict

PLR_PREFLIGHT_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = PLR_PREFLIGHT_ROOT.parent
FIXTURE_GRAPH_JSON = PLR_PREFLIGHT_ROOT / "tests" / "fixtures" / "simple_transfer_graph.json"
CONTRACTS_JSON = PLR_PREFLIGHT_ROOT / "data" / "derived_contracts.json"
...
# ---------------------------------------------------------------------------
# AC-6.1 -- import plr_preflight.check with BOTH libcst and pylabrobot poisoned.
# ---------------------------------------------------------------------------
...
def test_check_imports_without_libcst() -> None:
    ...
    reachable, not just unavailable), then `import plr_preflight.check`. Assert
    exit 0."""
    src_path = str(PLR_PREFLIGHT_ROOT / "src")
    preamble = (
        "import sys; "
        "sys.modules['libcst'] = None; "
        "sys.modules['pylabrobot'] = None; "
        "import plr_preflight.check"
    )
    result = subprocess.run(
        [sys.executable, "-c", preamble],
        cwd=str(PLR_PREFLIGHT_ROOT),
    ...
    assert result.returncode == 0, (
        f"import plr_preflight.check failed with libcst+pylabrobot poisoned:\n"
...
def test_fixture_exercises_contract_table_lookup(report: AnalysisReport) -> None:
    ...
    which all pass `plr_site=None` -- see `plr_preflight.check._unknown`)."""
...
def test_supported_tools_match_upstream() -> None:
    """`plr_preflight.check.SUPPORTED_TOOLS` (the single in-package definition,
    T8 consolidation -- see `plr_preflight/check/_supported_tools.py`) must be the
    ...
    `src/plr_preflight` cannot import `verify` (import-boundary test forbids it);
    ...
    assert SUPPORTED_TOOLS == upstream_dispatcher.SUPPORTED_TOOLS, (
        f"plr_preflight.check.SUPPORTED_TOOLS {sorted(SUPPORTED_TOOLS)} != "
...
def _git_state_from_dict(d: dict):
    from plr_preflight._provenance.git_state import GitState

    return GitState(**d)


def _stamp_from_dict(d: dict):
    from plr_preflight._provenance import SurveyStamp
...
def test_full_pipeline_emits_stamped_jsonl(
    ...
    ever called plr_preflight.telemetry.emit*; check_graph now does, for every
    ...
    test in this module."""
    from plr_preflight.telemetry import JsonlSink, set_sink
