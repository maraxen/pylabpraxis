...
here -- its comparison target (`plr_preflight/check/graph.py`) does not exist yet
...
Fork B (§5.2): `plr_preflight/_provenance/git_state.py` is a cherry-pick from
...
with a named reason when `$PLR_PREFLIGHT_CISTERNAL_ROOT` (default
...
from plr_preflight._provenance import git_state
...
def test_git_state_matches_cisternal() -> None:
    ...
    cisternal_root = Path(
        os.environ.get("PLR_PREFLIGHT_CISTERNAL_ROOT", str(_DEFAULT_CISTERNAL_ROOT))
    )
    if not cisternal_root.is_dir():
        pytest.skip(
            f"cisternal checkout not found at {cisternal_root} "
            f"(set $PLR_PREFLIGHT_CISTERNAL_ROOT to override)"
        )
...
#: `plr_preflight._hand_maintained.REGISTRY` (260901 T13) -- that row's `measure`
...
def test_plr_submodule_pin_matches_expected() -> None:
    ...
    assert state.hash == EXPECTED_SUBMODULE_PIN, (
        ...
        f"plr-preflight/tests/test_fork_drift.py (and HM-23's declared value in "
        f"plr_preflight._hand_maintained.REGISTRY, which measures this same "
