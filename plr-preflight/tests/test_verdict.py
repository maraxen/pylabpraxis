...
from plr_preflight.verdict import (
    REASON_VOCABULARY,
    AnalysisReport,
    Finding,
    PlrSite,
...
SRC_ROOT = Path(__file__).resolve().parents[1] / "src" / "plr_preflight"
...
def test_from_wire_maps_unrecognized_string_to_unknown() -> None:
    ...
    # `plr_preflight.verdict.join` and this package's own test round-trip helpers
...
def test_no_bool_protocol() -> None:
    ...
    anywhere under src/plr_preflight/. Deliberately not a runtime `assert not
...
def test_reason_vocabulary_closed_forward() -> None:
    ...
    construction site in src/plr_preflight/ resolves to a REASON_VOCABULARY
    member.

    Forward (unconditional): every `Finding(..., reason=...)` construction
    site in src/plr_preflight/ resolves to a REASON_VOCABULARY member; an
    ...
    synthetic snippets below, since src/plr_preflight/ has zero construction
    ...
    src/plr_preflight/ (plr_preflight.derive, which will construct them, is T6+), so
    ...
    # -- The real forward scan over src/plr_preflight/. Unconditional: with zero
...
def _scan_src_reason_sites() -> tuple[list[str], set[str], int]:
    """Shared scan over src/plr_preflight/ backing both directions of §3.3's
...
def test_reason_vocabulary_closed_reverse() -> None:
    ...
    reachable from >=1 `Finding(...)` construction site in src/plr_preflight/.

    Split out from the forward direction deliberately. At T3 there are zero
    construction sites (plr_preflight.derive, which will make them, is T6+), so
    ...
    if total_sites == 0:
        pytest.skip(
            "reverse vocabulary check unarmed: src/plr_preflight/ contains zero "
            "Finding(...) construction sites yet (plr_preflight.derive lands in "
