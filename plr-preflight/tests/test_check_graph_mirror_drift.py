...
walk is scoped to `src/plr_preflight/`) -- so, unlike anything under `src/`, this
file MAY import `praxis.backend.utils.plr_static_analysis.models`. Compares
`plr_preflight.check.graph`'s mirror dataclasses' field names against the live
...
def test_mirror_fields_match_operation_node(mirror_cls, mirror_name, upstream_ref) -> None:
    ...
    assert not unmatched, (
        f"plr_preflight.check.graph.{mirror_name} mirrors field(s) {sorted(unmatched)} that "
...
def test_mirror_field_set_matches_spec_table(mirror_cls, mirror_name, spec_fields) -> None:
    ...
    assert mirror_fields == spec_fields, (
        f"plr_preflight.check.graph.{mirror_name}'s field set {sorted(mirror_fields)} != "
