...
`plr_preflight._hand_maintained.REGISTRY` is the data; this module is the
...
`plr_preflight._hand_maintained.resolve_measure`, and the tests below assert that
...
cross-package-boundary import live in `plr_preflight._hand_maintained` itself.
...
- HM-1, HM-5, HM-11 are plain, trivially-importable module-level facts
  ...
  `_hand_maintained.py` -- except that module lives under `src/plr_preflight/`,
...
from plr_preflight._hand_maintained import REGISTRY, BUDGET_CAP, live_rows, resolve_measure
...
def _measure_hm1() -> int:
    """`praxis.*` is forbidden under `src/plr_preflight/` (test_import_boundary.py's
...
def _measure_hm5() -> int:
    """`training.*` is forbidden under `src/plr_preflight/` for the same reason
...
def _measure_hm11() -> int:
    """`praxis.*` is forbidden under `src/plr_preflight/` -- see HM-1."""
...
def _measure_hm23() -> int:
    ...
    `plr_preflight._hand_maintained.resolve_measure`'s docstring); not a hardcoded
...
def test_hand_written_contracts_content_is_pinned() -> None:
    ...
    assert live_hash == _PINNED_METHOD_CONTRACTS_SHA256, (
        ...
        "plr-preflight/tests/test_hand_maintained_ratchet.py "
