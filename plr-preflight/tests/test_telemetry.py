...
import-boundary walk scopes to src/plr_preflight/ only -- importing verify from a
test here is legal. Importing verify from src/plr_preflight/telemetry.py would
...
from plr_preflight._provenance.git_state import GitState, capture_git_state
from plr_preflight.verdict import Finding, PlrSite, Verdict
from plr_preflight import telemetry
from plr_preflight.telemetry import (
    FAILURE_CATEGORIES,
    JsonlSink,
    build_event,
    emit,
...
# AC-4.3) -- not a fact about plr-preflight itself, so if the submodule is ever
...
def test_categories_match_upstream() -> None:
    ...
    assert FAILURE_CATEGORIES == upstream_taxonomy.FAILURE_CATEGORIES, (
        f"plr_preflight.telemetry.FAILURE_CATEGORIES "
...
def test_event_carries_stamp(
    ...
    if case == "real_repo":
        ...
        from plr_preflight._provenance.git_state import _UNAVAILABLE
