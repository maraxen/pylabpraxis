"""AC-1 / Spec 260901 §1 import boundary: nothing under plr_preflight/src may import
...
T8 addition (spec §1.3/AC-6.2): `src/plr_preflight/check/` additionally may not
...
Round-4 remediation (M6): AC-1.1 used to be satisfied by "`import plr_preflight`
exits 0" alone -- true even when `plr_preflight/__init__.py` exported nothing
(`__all__ = []`), which was the actual pre-round-4 state:
`plr_preflight.check_graph` was unreachable from the top-level package despite
...
PLR_PREFLIGHT_ROOT = SRC_ROOT.parent.parent
...
def test_no_pylabrobot_import_under_check() -> None:
    """Spec §1.3/AC-6.2: `src/plr_preflight/check/` may import neither `pylabrobot`
...
def test_plain_cpython_import_of_public_surface() -> None:
    ...
    `import plr_preflight` must expose `check_graph`, `AnalysisReport`, and
    ...
    state, in which `plr_preflight.check_graph` was unreachable despite being the
    package's round-1 entry point (spec §6.2)."""
    src_path = str(PLR_PREFLIGHT_ROOT / "src")
    preamble = (
        "import plr_preflight; "
        "assert callable(plr_preflight.check_graph); "
        "assert plr_preflight.AnalysisReport is not None; "
        "assert plr_preflight.Verdict is not None"
    )
    ...
    assert result.returncode == 0, (
        f"import plr_preflight (public surface) failed:\n"
