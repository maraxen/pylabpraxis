...
- **The seed `CALL` does contribute one finding of its own, and it is not the fallback.**
  ...
  emits one `_findings_for_gap` finding per gap (`plr-sema/src/plr_sema/check/__init__.py:316-324`), so
