...
`SurveyRecord.dropped_calls` (`derive/__init__.py:174-192`) — the receiver-qualified call expressions
...
| **A-COMPLETES** | each operation preceding the one being checked completed without raising | E1/E2's post-state is the state *after a successful* call | it is the same assumption the oracle's own comparison already makes: `oracle_common.compare` marks every operation after the failing index `not_reached` and imposes no constraint there (`plr-sema/eval/oracle_common.py:592-611`; the round-1 draft cited `:148-163`, which is `run_static`'s per-operation grouping, not the comparison). A `WILL_FAIL` at index `i` is a claim about the trace *reaching* `i` |
...
- **AC-10.11 (oracle gate — replay; vacuous under tier 1 as configured, and says so).** `#4879`'s
  ...
  `oracle_common.compare`'s own `unsound` predicate (`plr-sema/eval/oracle_common.py:592-611`): no
