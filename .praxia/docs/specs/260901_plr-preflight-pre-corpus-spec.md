...
# Specification: plr-preflight, pre-corpus (round 6)

> **Renamed 260902:** this package was `plr-jit` through spec_version 8. "JIT" was a misnomer — nothing is
> compiled just-in-time; it is a pre-execution sound static check, hence `plr-preflight` (import
> `plr_preflight`). Historical task_ids (`260901_plr-jit-*`, `260901_plr_jit_spec`) are identifiers and
...
1. **Does `Verdict` gain a fourth, `UNREACHABLE` member now? — No, not now.** ⊥ (bottom/unreachable) is
   ...
   `Verdict.from_wire(value: str) -> Verdict` in `plr_preflight/verdict.py`, tested in
   ...
   **Corrects §3.5's schema-bump direction, which had it backwards for an additive enum member:** for
   ...
   consumers outside `plr-preflight/tests` import `plr_preflight.verdict` or read a report `schema_version`
...
2. **Is volume tracking in the v1 verdict path? — The question was mis-posed; restated below, and the
   ...
   `plr-preflight/data/derived_contracts.json`; the whole-surface table carries 257 volume/liquid-shaped
   ...
   **Resolution: leave numeric atoms at ½ through v1 and the first post-corpus increment.** This is
   ...
   discussion in deferred row (b)/`260901_plr-preflight-research-b-f.md`) before it can be honest, which is itself
...
3. **Does the `join` lattice ordering invert so `UNKNOWN` is genuinely top? — No; keep the table.** The
   ...
   **A related, more serious correction, recorded prominently so a later round does not resurrect
   it:** `260901_plr-preflight-research-a-d.md`'s R3 recommended grouping `join`'s input by `operation_id` and
   ...
   `260901_plr-preflight-research-a-d.md` both had this backwards; do not implement R3 as written, and do not re-raise the
...
`plr-preflight` v1's eight sections are corpus-*independent* plumbing. Wire them all together and you get a
...
`plr_preflight.verdict.join` returns `UNKNOWN` on the empty finding multiset (§3.2, unconditionally as of
...
```
plr-preflight/
  pyproject.toml           # name = "plr-preflight", requires-python = ">=3.10"
  src/plr_preflight/
...
Root `pyproject.toml` gains `"plr-preflight"` to `[tool.uv.workspace] members`.

**`plr-preflight/pyproject.toml` MUST carry its own `[tool.pytest.ini_options]` with
`addopts = ["--no-cov"]` and `testpaths = ["tests"]`.** Without it the package inherits the root
`addopts` (`pyproject.toml:158-174`), which includes `--cov=praxis`; running pytest inside `plr-preflight/`
...
> direction-agnostic. The naive reading — `plr_preflight` re-exports from `praxis` — is **structurally
...
> **praxis → plr_preflight**.

**Normative:** the dependency arrow points `praxis → plr_preflight`, never the reverse. When a symbol
...
from plr_preflight.<mod> import *   # noqa: F403  -- compatibility shim, see spec 260901
from plr_preflight.<mod> import __all__  # noqa: F401
```
...
```bash
uv run pytest plr-preflight/tests/test_import_boundary.py -q
```
...
- `test_no_praxis_imports_under_src` — walks every node of every `.py` under `src/plr_preflight/`, asserts
...
- `test_no_pylabrobot_import_under_check` — same walk restricted to `src/plr_preflight/check/`, asserting
  no `pylabrobot` and no `libcst` top-level import (§6's packaging fact, mechanised).
- `test_plain_cpython_import_of_public_surface` — subprocess `python -c "import plr_preflight"`, asserting
  exit 0.
...
- **AC-1.1** `uv sync` at repo root resolves with `plr-preflight` as a workspace member; `uv run python -c
  "import plr_preflight"` exits 0.
- **AC-1.2** `uv run pytest plr-preflight/tests/test_import_boundary.py -q` passes, and passes **as a
  meaningful test**: a scratch commit adding `import praxis` to any `src/plr_preflight/` module makes it
  fail. (Demonstrate once; do not commit the scratch.)
- **AC-1.3** `uv run pytest plr-preflight -q` runs without inheriting `--cov=praxis` (assert by observing no
...
`plr-preflight/src/plr_preflight/_provenance/git_state.py`, prepending a provenance header (below) and changing
nothing else.
...
1. **`GIT_DIR` stripping** (`_GIT_REPO_LOCATION_ENV_VARS`, `git_state.py:46-59`). Six env vars
   override `-C <path>` repo discovery. Without stripping, a `plr-preflight` survey run from inside a git
...
# DO NOT EDIT. Drift is enforced by plr-preflight/tests/test_provenance_drift.py.
# ---------------------------------------------------------------------------
```

### 2.2 The stamp schema it feeds

`plr_preflight._provenance.stamp.survey_stamp() -> SurveyStamp`, a frozen dataclass, superseding the
...
```bash
uv run pytest plr-preflight/tests/test_provenance.py -q
```
...
`import plr_preflight` in Pyodide and possibly on 3.10. `test_git_state_is_stdlib_only` converts this into
...
- **AC-2.5** Root `pyproject.toml`'s ruff `exclude` list (`pyproject.toml:215-220`) includes
  `plr-preflight`, or `plr-preflight/.ruff.toml` independently excludes `_provenance/git_state.py` — asserted by
  running `pre-commit run ruff --files plr-preflight/src/plr_preflight/_provenance/git_state.py` once locally and
...
deserialized verbatim from whatever `derived_contracts.json` already recorded when `plr_preflight.derive`
...
count-only form, does not imply the surjective per-operation claim; `260901_plr-preflight-research-a-d.md:348-354`,
...
> `plr_preflight.verdict.join(findings) -> Verdict`, with no other call site allowed to aggregate. **What
...
> **reachability** fact (`260901_plr-preflight-research-a-d.md:126-134`) — the anticipated extension point is a narrow
...
> `260901_plr-preflight-research-a-d.md:156-161` explicitly prefers the narrow-parameter form for exactly this reason).
...
sense that each reason corresponds to exactly one `return`-with-no-result site in `plr_preflight.derive`.
...
construction in `src/plr_preflight/`, resolving the `reason=` argument as either a string literal or a
...
```bash
uv run pytest plr-preflight/tests/test_verdict.py -q
```
...
- `test_join_absorbs_across_shared_operation_id` — **new, round-4 remediation (M7).** None of the
  ...
  `260901_plr-preflight-research-a-d.md` flags as sound-today-but-not-post-(a): `join((SAFE@op1, WILL_FAIL@op1))` absorbs
...
- `test_no_bool_protocol` — `ast.walk` over `src/plr_preflight/`, assert no `__bool__` def on `Verdict`,
...
- `test_reason_vocabulary_closed` — AST scan of every `Finding(...)` construction in
  `src/plr_preflight/`; every `reason=` argument, resolved as a literal or a module-level constant, ∈
...
`Verdict.from_wire`, `plr_preflight/verdict.py`, tested in `test_verdict.py`) — mapping an unknown value to
...
- **AC-3.2** `grep -rn "__bool__" plr-preflight/src/` returns nothing.
...
- **AC-3.4** (reworded, D15) An `AnalysisReport` **constructed directly** (as
  ...
  pipeline to run.
- **AC-3.5** (round-4 remediation, B1/B2, fix 6) `plr_preflight.verdict.join(())` (the empty finding tuple)
...
| category | dynamic meaning (verify/) | static meaning (plr-preflight) |
|---|---|---|
...
```bash
uv run pytest plr-preflight/tests/test_telemetry.py -q
```

- `test_categories_match_upstream` — imports `FAILURE_CATEGORIES` from **both**
  `plr_preflight.telemetry` and `verify.failure_taxonomy`, asserts set equality. This is a live
...
> shape, not `plr_preflight`'s `SurveyStamp` — `training/` gains no `plr_preflight` import to satisfy the literal
...
- **AC-4.4** `plr_preflight` imports and analyzes with `set_sink(None)` — telemetry is strictly optional.
...
**Interface:** `plr-preflight/tests/test_fork_drift.py::test_coxswain_port_matches_upstream` +
`plr-preflight/tests/test_fork_drift.py::test_every_ported_module_is_covered`.
...
- **Tier 2 — upstream comparison (skips when cisternal is absent).** If
  `$PLR_PREFLIGHT_CISTERNAL_ROOT` (default `/home/marielle/projects/cisternal`) exists, read the upstream
...
static file snapshot, and the "fork" is `plr_preflight/check/graph.py`'s stdlib-dataclass mirror (§6.2,
...
**Resolution.** `plr-preflight/tests/test_check_graph_mirror_drift.py::test_mirror_fields_match_operation_node`
— living under `plr-preflight/tests/`, which §1.3's boundary walk does **not** cover (that walk is scoped
to `src/plr_preflight/`), so this test file may import
...
```bash
uv run pytest plr-preflight/tests/test_fork_drift.py -q -rs   # -rs surfaces skip reasons
```
...
```bash
uv run pytest plr-preflight/tests/test_check_graph_mirror_drift.py -q
```
...
- **AC-5.2** `uv run pytest plr-preflight/tests/test_fork_drift.py -q` with `PLR_PREFLIGHT_CISTERNAL_ROOT`
...
right shape; plr-preflight makes it a **packaging fact** rather than a convention.
...
```
src/plr_preflight/extract/     SERVER-SIDE.  libcst permitted. pylabrobot import permitted. ROUND 2 — see below.
                         Input: protocol source. Output: ProtocolComputationGraph (JSON).
src/plr_preflight/check/       BROWSER-SIDE. NO libcst. NO pylabrobot. stdlib only (no pydantic — see below).
                         Input: graph JSON + derived contract table JSON + SurveyStamp.
                         Output: AnalysisReport (JSON).
src/plr_preflight/derive/      BUILD-TIME. §7's derivation pipeline. libcst not required; reads the
...
**`plr_preflight.check` never imports `pydantic`.** `OperationNode`, `ResourceNode`,
`ProtocolComputationGraph` (`plr_static_analysis/models.py:524-661`) are pydantic `BaseModel`s under
`praxis.*` — forbidden by §1.3 (no `praxis` import under `src/plr_preflight/`) and unmovable per §1.1
...
types it actually reads, in `plr_preflight/check/graph.py`. This is mechanical, not a design choice
...
- **Over-inclusive (M1, CONCEDE):** `line_number`, `node_type`, and `arguments` were mirrored but
  ...
  `src/plr_preflight/` tree. `line_number` was `0` for all four fixture operations, so a `PlrSite` built
...
- `condition_expr`/`true_branch`/`false_branch` genuinely have no round-1 consumer, but they are named
  ...
  declaration; `260901_plr-preflight-research-a-d.md:136-146`) — i.e. they are the field-set cost of eventually building
...
**copied verbatim** into `plr_preflight/check/`, exactly as §4.1 copies `FAILURE_CATEGORIES` rather than
...
`plr_preflight.check` and `training.verify.dispatcher`, since the import-boundary restriction is scoped to
`src/plr_preflight/` per §1.3) asserts set equality, skipped with an explicit reason if `training.verify` is
not importable.

**Redefined, 260901 T11: `unsupported_tool` no longer checks `SUPPORTED_TOOLS` membership.** T11
decouples §7's derivation pipeline from `SUPPORTED_TOOLS` entirely — `plr_preflight.derive` now derives a
...
still shipped — `plr_preflight.check.SUPPORTED_TOOLS` still exists, is still re-exported, and
`test_supported_tools_match_upstream` still passes — but the set is now purely informational
(`plr_preflight.derive.build_gap_ledger`'s `supported_tools`-scoped reporting subset) plus the one drift
test; it no longer gates which methods get a derived contract or which `Finding.reason` an operation
receives. See HM-9 (§9.2) for the registry-row restatement and `plr_preflight.check`'s module docstring
...
`plr-preflight/tests/fixtures/`. Producing that fixture is itself round-1 work (see T8 below); it is not an
`ast`-visible `praxis` import from `src/plr_preflight/`, so §1.3 stays intact.
...
```bash
uv run pytest plr-preflight/tests/test_import_boundary.py::test_no_pylabrobot_import_under_check -q
uv run python -c "import plr_preflight.check"   # in an env where libcst is NOT installed
```
...
- `test_check_imports_without_libcst` — spawn a subprocess with `sys.modules['libcst'] = None` and
  ...
  poisons both entries), then `import plr_preflight.check`. Assert exit 0. Poisoning is stronger than
...
- `test_supported_tools_match_upstream` — **new, D1.** Imports `SUPPORTED_TOOLS` from both
  `plr_preflight.check` (the copy) and `training.verify.dispatcher` (upstream), asserts set equality. Same
...
- **AC-6.1** `import plr_preflight.check` succeeds in a process where both `libcst` and `pylabrobot` are
...
- **AC-6.4** (round-4 remediation, B2, strengthened from subset to surjectivity, fix 6) For the
  ...
  simultaneously while silently never reporting on the rest (`260901_plr-preflight-research-a-d.md:348-354`).
...
bridge requires loading it, via a required `--taxonomy-json PATH` on `plr_preflight.differential`, no
...
Serialized to `plr-preflight/data/derived_contracts.json`, a **build artifact, never hand-edited**:
...
disambiguator (`build_contract_keys`, `plr_preflight.derive`): if `qualname` is unique among the population
...
uv run python -m plr_preflight.derive \
    ...
    --out plr-preflight/data/derived_contracts.json
```

**`--survey-json PATH` is required, with no default (D19).** `derive/` is a workspace member
forbidden to import `praxis` (§1.3's boundary is scoped to `src/plr_preflight/`, but `derive/`'s own
...
uv run python -m plr_preflight.derive \
    --survey-json training/verify/data/plr_preconditions.json \
    --gap-ledger plr-preflight/data/gap_ledger.json
```
...
```bash
uv run pytest plr-preflight/tests/test_derive.py -q
uv run python -m plr_preflight.derive \
    --survey-json training/verify/data/plr_preconditions.json \
...
- `test_ledger_regenerates_deterministically` — **new, round-4 remediation, m2, CONCEDE.** Two
  ...
  had no test behind it at all (grepped `plr-preflight/tests/` for byte-identity/determinism checks: zero
  hits, pre-round-4).
...
`plr-preflight` v1 is sound but empty. **This is the single biggest content risk in the document** (RISK-1).
...
- **AC-7.2 (totality, whole-surface as of 260901 T11 — was `SUPPORTED_TOOLS`-only, T6-only, through
  ...
  (`plr_preflight.derive.__main__`) now iterates `build_unique_index(records)` (every record individually,
...
`--taxonomy-json`; **never imported** — `src/plr_preflight/` may not import `praxis`, §1.3/AC-1.2), against
...
`plr-preflight/pyproject.toml` declares `dependencies = []` and §1.2's arrow says the package does not reach
...
```bash
uv run python -m plr_preflight.differential \
...
uv run pytest plr-preflight/tests/test_differential.py -q
```
...
hand-maintained fact in `plr-preflight` gets a registry row, a size metric, and a test that fails when the
measured size exceeds the declared size.**

### 9.1 The registry

`plr-preflight/src/plr_preflight/_hand_maintained.py` — a data module, imported by the ratchet test:
...
| HM-9 | `SUPPORTED_TOOLS` (`dispatcher.py`) | tools | **10** | CAPPED (10) | **Redefined 260901 T11**: through spec_version 6 this row doubled as both `training.verify.dispatcher`'s dynamic-execution capability limit AND `plr_preflight`'s entire analyzed surface, because both derivation and `unsupported_tool` were hand-gated on it. T11 decouples the two: `plr_preflight.derive` now derives a contract for every method the survey indexes (4,770 at the current pin, §7.3/§7.6), and `unsupported_tool` means "key absent from that whole-survey contract table" (§6.2), not "outside this 10-tool set". This row is UNCHANGED and still real — it remains `training.verify.dispatcher`'s own dynamic-execution-harness scope boundary (what it can actually dispatch at runtime), a fact about OUR harness, not PLR — but it no longer gates what `plr_preflight` derives or checks. None known — growth of the *harness's* boundary is still a deliberate scope decision made in `verify/`, orthogonal to `plr_preflight`. |
...
| HM-23 | `EXPECTED_SUBMODULE_PIN`, Fork D's expected-pin literal (`plr-preflight/tests/test_fork_drift.py`, added 260901 T13 — live row 22) | declared pin literals | **1** | FROZEN | None — a fixed expectation is the whole point of a drift test; deriving it from the submodule would make the check vacuous (same reasoning as HM-17/HM-18). |

**MEASURE** = a one-off helper (`uv run python -m plr_preflight._hand_maintained --update-baselines`, run
...
```bash
uv run pytest plr-preflight/tests/test_hand_maintained_ratchet.py -q
```
...
| (b1) | The formal meaning of `UNKNOWN` — sound over-approximation vs. bail-out, i.e. how `UNKNOWN` should PROPAGATE (⊤-propagation semantics) | Genuinely presupposes (a): propagation needs an abstract state to propagate, which does not exist pre-(a). **(round-4 remediation, B3 row (b), split — the stated reason survives:** `260901_plr-preflight-research-b-f.md:35` opens "it is real, but the spec has named the wrong axis" about the UNDIVIDED row, not about this half.) |
| (b2) | Whether an `UNKNOWN`-worthy obligation is generated AT ALL, for a construct the front end cannot yet classify | **Not blocked on (a) — in scope for T6's recording rule now (round-4 remediation, B3 row (b), split, CONCEDE half).** A front-end/TCB recording-completeness property, not a propagation-semantics one; the literature prescribes a fail-closed front end (`260901_plr-preflight-research-b-f.md:53-59,100-104`). Silently dropping the obligation (rather than recording SOME `UNKNOWN` reason for it) was the actual gap — category (C) in `260901_plr-preflight-research-b-f.md`. |
...
| RISK-2 | Shim direction inverted — `plr_preflight` imports `praxis` — making the boundary test unsatisfiable (§1.2). | low (now flagged) | high | §1.2 makes the arrow normative; the day-one boundary test converts a violation into a red test on the first offending commit rather than after N modules have moved. |
...
rollback is `git rm -r plr-preflight/` and reverting the one added line in root `pyproject.toml`. No
...
1. **Decision 1's shim is direction-ambiguous and only one direction is implementable.** "A
   ...
   `plr_preflight` re-exporting from `praxis`, which is structurally incompatible with the day-one
   boundary test. §1.2 resolves it normatively as `praxis → plr_preflight`, but the resolution is mine,
...
- **C10** — Elevated to normative, tested T2 scope: added AC-2.5 and a T2 deliverable requiring
  `plr-preflight` be excluded from root `pyproject.toml`'s ruff `exclude` (or a `plr-preflight/.ruff.toml`), since
...
gap ledger; `praxis → plr_preflight` shim direction), and none of the six literature-corpus-deferred items
was specified.
...
- **D8** — §5 (new §5.3 "Fork C"), §9.2, T8: rebutted the claim that §6.2:828's "no third model
  ...
  under `plr-preflight/tests/` (exempt from §1.3's `src/plr_preflight/`-scoped boundary walk) so it may import
...
- **D19** — §6.2/§7.3/§7.4/T6: specified a required `--survey-json PATH` (no default) on every
  `python -m plr_preflight.derive` invocation; §7.1/T10: specified a required `--taxonomy-json PATH` on
  `python -m plr_preflight.differential`, and added it to T10's row and gate command.
...
| **M6** | MAJOR | `check_graph`, `Verdict`, `AnalysisReport` exported; AC-1.1 strengthened from "`import plr_preflight` exits 0" to asserting importability of the declared surface. |
...
(assert `consumed ⊆ upstream`, let the mirror lead) would give a plr-preflight-invented field a permanent
...
argument as made, and does not apply here.** Round 5's F4 argued for widening `plr_preflight`'s analyzed
...
from the user, who is the one party who can actually make a fitness call for `plr_preflight`'s intended
...
1. `plr_preflight.derive.__main__.build_derived_contracts_payload` now derives a contract for every record
...
3. `unsupported_tool` (§3.3) is redefined: "key absent from the whole-survey contract table," not
   ...
   (`plr_preflight.check.SUPPORTED_TOOLS`, `test_supported_tools_match_upstream`) — it is now purely
   `training.verify.dispatcher`'s own dynamic-execution-harness capability boundary (HM-9, §9.2), no
   longer `plr_preflight`'s analyzed-surface boundary.
...
**Tests changed:** `plr-preflight/tests/test_derive.py` gains
`test_contract_keys_are_collision_free`/`test_whole_surface_contract_count`;
`plr-preflight/tests/test_check_graph.py` gains
...
1. **`UNREACHABLE`: not added.** Reserve the wire string `"unreachable"` (comment only, unused); add
   ...
   as `Verdict.from_wire(value: str) -> Verdict` in `plr_preflight/verdict.py`. §3.5's schema-bump direction
...
3. **`join` ordering: not inverted.** The shipped table is the correct join of the obligation order;
   ...
   docstring name both orders explicitly. **`260901_plr-preflight-research-a-d.md`'s R3 (group `join`'s input by
...
```
uv run pytest plr-preflight/tests/test_verdict.py -q
uv run pytest plr-preflight/tests/test_check_graph.py -q
uv run pytest plr-preflight/tests/test_derive.py -q
uv run pytest plr-preflight/tests/test_telemetry.py -q
uv run pytest plr-preflight/tests/test_provenance.py -q
uv run pytest plr-preflight/tests/test_fork_drift.py -q
uv run pytest plr-preflight/tests/test_import_boundary.py -q
uv run pytest plr-preflight/tests/test_check_graph_mirror_drift.py -q
