...
from praxis.backend.services.praxis_orm_service import (
  PraxisDBService,
)  # Keep for mock_praxis_db_service
...
from praxis.backend.core.run_context import PraxisState, Deck, Resource
...
async def mock_praxis_db_service(db_session, mock_asyncpg_pool):
  ...
  with patch(
    "praxis.backend.services.praxis_orm_service.PraxisDBService.initialize",
    new_callable=AsyncMock,
  ) as mock_init, patch(
    "praxis.backend.services.praxis_orm_service.PraxisDBService.get_praxis_session"
  ) as mock_get_session:
...
async def discovery_service(
  db_session: AsyncSession, mock_praxis_db_service
):  # mock_praxis_db_service ensures patches are active
  """
  ...
  if hasattr(service, "initialize_discovery") and asyncio.iscoroutinefunction(
...
class TestProtocolDiscoveryService:
  async def test_discover_and_register_protocols_from_repo(
    self, discovery_service: ProtocolDiscoveryService, db_session: AsyncSession
  ):
    ...
    protocol_def_1 = FunctionProtocolDefinition(
      name="MyDecoratedProtocol",
      version="1.0.0",
      description="A test protocol.",
      module_name="temp_protocols_test_pkg.decorated_protos",
      function_name="decorated_protocol_func",
      parameters=[
        ParameterDefinition(
          name="state",
          type_hint_str="PraxisState",
          actual_type_str="PraxisState",
          optional=False,
          is_deck_param=False,
        ),
        ParameterDefinition(
    ...
    protocol_def_2 = FunctionProtocolDefinition(
      ...
      version="0.1.0",
      description=None,  # No explicit description in decorator
      module_name="temp_protocols_test_pkg.decorated_protos",
    ...
    )  # Must return Path object or string
    discovery_service._scan_directory_for_protocols = MagicMock(
      return_value=mock_discovered_protos_list
    )
    ...
    found_protocol_1 = next(
      (p for p in new_defs if p.definition.name == "MyDecoratedProtocol"), None
    )
    ...
    if found_protocol_1:
      ...
      assert any(
        p.name == "samples" and p.default_value_repr == "8"
        for p in found_protocol_1.definition.parameters
      )
      assert any(
        p.name == "deck" and p.is_deck_param
        for p in found_protocol_1.definition.parameters
      )
    ...
    found_protocol_2 = next(
      (p for p in new_defs if p.definition.name == "AnotherProtocol"), None
    )
    ...
    discovery_service._scan_directory_for_protocols.assert_called_once_with(
      Path(
        str(repo_orm.local_checkout_path)
      ),  # Ensure Path object if that's what _scan expects
      ProtocolSourceInfo(
        source_type=ProtocolSourceType.GIT_REPOSITORY,
    ...
    discovery_service._register_discovered_protocols.assert_called_once_with(
      mock_discovered_protos_list, repo_orm, None
    )

  async def test_discover_from_decorated_module_via_fs_source(
    self,
    discovery_service: ProtocolDiscoveryService,
    db_session: AsyncSession,
    temp_protocol_dir: Path,
  ):
    """
    ...
    create_dummy_protocol_file(
      temp_protocol_dir, module_name, PROTOCOL_CONTENT_DECORATED
    )
    ...
    fs_source_orm = FileSystemProtocolSourceOrm(
      ...
      base_path=str(
        temp_protocol_dir.parent
      ),  # The directory *containing* 'temp_protocols_test_pkg'
    )
    ...
    protocol_def_1 = FunctionProtocolDefinition(
      ...
      version="1.0.0",
      description="A test protocol.",
      module_name=f"temp_protocols_test_pkg.{module_name}",
      function_name="decorated_protocol_func",
      parameters=[
        ParameterDefinition(
          name="state",
      ...
      assets=[],
      is_top_level=True,
      source_file_path=f"{module_name}.py",  # Relative to package
      docstring="This is a docstring for decorated_protocol_func.",
    )
    protocol_def_2 = FunctionProtocolDefinition(
      name="AnotherProtocol",
