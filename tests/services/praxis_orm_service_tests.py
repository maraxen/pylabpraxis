...
from praxis.backend.utils.db import Base as PraxisBase # Renamed to avoid conflict if Base is used locally
from praxis.backend.services.praxis_orm_service import PraxisDBService, _get_keycloak_dsn_from_config
...
async def praxis_db_service(db_session, mock_asyncpg_pool):
    ...
    with patch("praxis.backend.services.praxis_orm_service.AsyncSessionLocal", return_value=db_session), \
         patch("praxis.backend.services.praxis_orm_service._get_keycloak_dsn_from_config", return_value="postgresql://mock:mock@mock/mock") as mock_get_dsn, \
         patch("asyncpg.create_pool", return_value=mock_asyncpg_pool) as mock_create_pg_pool:

        service_instance = await PraxisDBService.initialize(keycloak_dsn="postgresql://mock:mock@mock/mock")
        # The initialize method should now use the mocked create_pool

        # Ensure the service instance actually got the mocked pool
        assert service_instance._keycloak_pool is mock_asyncpg_pool, "Service did not use the mocked Keycloak pool"
...
async def create_test_user(session: AsyncSession, user_id: str, username: str = "testuser") -> UserOrm:
    user = UserOrm(id=user_id, username=username, email=f"{username}@example.com", first_name="Test", last_name="User")
    session.add(user)
    await session.commit()
    await session.refresh(user)
    return user

async def create_test_protocol_source_repo(session: AsyncSession, name: str = "TestRepo") -> ProtocolSourceRepositoryOrm:
    repo = ProtocolSourceRepositoryOrm(name=name, git_url="http://example.com/repo.git", default_ref="main")
    session.add(repo)
    await session.commit()
...
async def create_test_labware_definition(session: AsyncSession, name: str = "test_plate_def_96") -> LabwareDefinitionCatalogOrm:
    lw_def = LabwareDefinitionCatalogOrm(
...
class TestProtocolRunManagement:
    ...
    async def test_register_protocol_run(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        ...
        proto_def = await create_test_protocol_definition(db_session, source_repo_id=repo.id)
    ...
    async def test_get_protocol_run_details_exists(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        """Test retrieving details of an existing protocol run."""
        test_user_id = str(uuid.uuid4())
        user = await create_test_user(db_session, user_id=test_user_id, username="details_user")
        repo = await create_test_protocol_source_repo(db_session)
        proto_def = await create_test_protocol_definition(db_session, source_repo_id=repo.id)
    ...
    async def test_get_protocol_run_details_not_exists(self, praxis_db_service: PraxisDBService):
    ...
    async def test_update_protocol_run_status(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        ...
        await praxis_db_service.update_protocol_run_status(run_id, ProtocolRunStatusEnum.RUNNING)
        ...
        await praxis_db_service.update_protocol_run_status(run_id, ProtocolRunStatusEnum.COMPLETED)
    ...
    async def test_list_protocol_runs(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        ...
        repo = await create_test_protocol_source_repo(db_session)
        proto_def1 = await create_test_protocol_definition(db_session, source_repo_id=repo.id, name="ProtoDef1")

        # Run 1 (User1, Pending)
        await praxis_db_service.register_protocol_run(proto_def1.id, user1_id, {"p":1}, ProtocolRunStatusEnum.PENDING)
        # Run 2 (User1, Running)
        await praxis_db_service.register_protocol_run(proto_def1.id, user1_id, {"p":2}, ProtocolRunStatusEnum.RUNNING)
        # Run 3 (User2, Completed)
        await praxis_db_service.register_protocol_run(proto_def1.id, user2_id, {"p":3}, ProtocolRunStatusEnum.COMPLETED)
        ...
        pending_runs = await praxis_db_service.list_protocol_runs(status=ProtocolRunStatusEnum.PENDING)
        assert len(pending_runs) == 1
        assert pending_runs[0]["status"] == ProtocolRunStatusEnum.PENDING.name

        user2_completed_runs = await praxis_db_service.list_protocol_runs(status=ProtocolRunStatusEnum.COMPLETED, created_by_user_id=user2_id)
        ...
        empty_runs = await praxis_db_service.list_protocol_runs(status=ProtocolRunStatusEnum.FAILED)
        assert len(empty_runs) == 0


@pytest.mark.asyncio
class TestAssetInstanceManagement:
    """Tests for asset (LabwareInstanceOrm) management methods."""

    async def test_add_asset_instance_new(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        """Test adding a new labware instance."""
        lw_def_name = "my_test_plate_def"
        await create_test_labware_definition(db_session, name=lw_def_name)
        ...
        async with praxis_db_service.get_praxis_session() as verify_session:
            stmt = select(LabwareInstanceOrm).where(LabwareInstanceOrm.id == asset_id)
            result = await verify_session.execute(stmt)
        ...
        assert asset_orm.current_status == LabwareInstanceStatusEnum.AVAILABLE_IN_STORAGE

    async def test_add_asset_instance_update(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        """Test updating an existing labware instance."""
        ...
        updated_id = await praxis_db_service.add_asset_instance(
            ...
            current_status=LabwareInstanceStatusEnum.IN_USE,
            lot_number="LOT123"
        )
        ...
        assert details["current_status"] == LabwareInstanceStatusEnum.IN_USE.name
        assert details["lot_number"] == "LOT123"

    async def test_get_asset_instance_exists(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        """Test retrieving an existing labware instance."""
        ...
        assert details["current_status"] == LabwareInstanceStatusEnum.AVAILABLE_ON_DECK.name
        assert details["is_available"] is True

    async def test_get_asset_instance_not_exists(self, praxis_db_service: PraxisDBService):
        """Test retrieving a non-existent labware instance."""
        details = await praxis_db_service.get_asset_instance("NonExistentPlate999")
        assert details is None

    async def test_asset_availability_logic(self, praxis_db_service: PraxisDBService, db_session: AsyncSession):
        ...
        # Available statuses
        await praxis_db_service.add_asset_instance(asset_available_names[0], lw_def_name, current_status=LabwareInstanceStatusEnum.AVAILABLE_IN_STORAGE)
        await praxis_db_service.add_asset_instance(asset_available_names[1], lw_def_name, current_status=LabwareInstanceStatusEnum.AVAILABLE_ON_DECK)
        await praxis_db_service.add_asset_instance(asset_available_names[2], lw_def_name, current_status=LabwareInstanceStatusEnum.EMPTY) # Assuming EMPTY is available

        # Unavailable statuses
        await praxis_db_service.add_asset_instance(asset_unavailable_names[0], lw_def_name, current_status=LabwareInstanceStatusEnum.IN_USE)
        await praxis_db_service.add_asset_instance(asset_unavailable_names[1], lw_def_name, current_status=LabwareInstanceStatusEnum.ERROR)
        ...
        for name in asset_available_names:
            ...
            assert details["is_available"] is True, f"Asset {name} with status {details['current_status']} should be available"

        for name in asset_unavailable_names:
            details = await praxis_db_service.get_asset_instance(name)
            assert details is not None, f"Details for {name} should not be None"
            assert details["is_available"] is False, f"Asset {name} with status {details['current_status']} should NOT be available"
...
class TestKeycloakUserManagement:
    ...
    async def test_get_all_users_success(self, praxis_db_service: PraxisDBService, mock_asyncpg_pool: AsyncMock):
        """Test fetching all users from Keycloak successfully."""
        mock_users_data = [
            {"id": "user-id-1", "username": "kuser1", "email": "kuser1@example.com", "first_name": "Keycloak", "last_name": "UserOne", "enabled": True},
            {"id": "user-id-2", "username": "kuser2", "email": "kuser2@example.com", "first_name": "Keycloak", "last_name": "UserTwo", "enabled": True},
        ]
    ...
    async def test_get_all_users_no_keycloak_pool(self, db_session: AsyncSession):
        ...
        with patch("praxis.backend.services.praxis_orm_service.AsyncSessionLocal", return_value=db_session), \
             patch("praxis.backend.services.praxis_orm_service._get_keycloak_dsn_from_config", return_value=None), \
             patch("asyncpg.create_pool", AsyncMock(side_effect=RuntimeError("Simulated pool creation failure"))): # Ensure create_pool is not called or fails

            service_no_kc = PraxisDBService() # Create instance without classmethod initialize
