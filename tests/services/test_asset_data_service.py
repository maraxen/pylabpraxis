...
from praxis.backend.api.assets import LabwareInventoryDataIn

# For flag_modified
from sqlalchemy.orm.attributes import flag_modified


class TestLabwareInstanceInventory:
    @pytest.fixture
    ...
    def test_add_labware_instance_with_inventory_properties(self, mock_db_session, caplog):
        mock_labware_def = MagicMock(spec=LabwareDefinitionCatalogOrm)
        with patch('praxis.backend.services.asset_data_service.get_labware_definition', return_value=mock_labware_def) as mock_get_def:
            
            sample_inventory_data = {
                "praxis_inventory_schema_version": "1.0",
                "reagents": [{"reagent_id": "R123", "initial_quantity": {"value": 100, "unit": "mL"}, "current_quantity": {"value": 90, "unit": "mL"}}],
                "item_count": {"item_type": "tip", "initial_max_items": 96, "current_available_items": 96},
                "consumable_state": "new",
                "inventory_notes": "Fresh tips"
            }

            instance = asset_data_service.add_labware_instance(
                db=mock_db_session,
                user_assigned_name="TestLWInventory",
                pylabrobot_definition_name="test_plate_def",
                properties_json=sample_inventory_data,
                initial_status=LabwareInstanceStatusEnum.AVAILABLE_IN_STORAGE
            )
            ...
            # The actual LabwareInstanceOrm object is created inside the function
            # So we need to check the object that db.add was called with.
            assert mock_db_session.add.call_count == 1
            added_instance_orm = mock_db_session.add.call_args[0][0] # Get the first positional argument of the first call
            
            assert isinstance(added_instance_orm, LabwareInstanceOrm)
            ...
            assert added_instance_orm.current_status == LabwareInstanceStatusEnum.AVAILABLE_IN_STORAGE
            
            mock_db_session.commit.assert_called_once()
            mock_db_session.refresh.assert_called_once_with(added_instance_orm)

            # Check log message
            assert f"DEBUG: ADS: Setting properties_json for new labware instance 'TestLWInventory': {sample_inventory_data}" in caplog.text

    @patch('praxis.backend.services.asset_data_service.flag_modified') # Patch where it's used
    def test_update_labware_instance_properties_json(self, mock_flag_modified, mock_db_session, caplog):
        mock_instance = MagicMock(spec=LabwareInstanceOrm)
        mock_instance.id = 1
        mock_instance.properties_json = {"existing_key": "old_value", "consumable_state": "used"}

        with patch('praxis.backend.services.asset_data_service.get_labware_instance_by_id', return_value=mock_instance) as mock_get_instance:
            
            properties_update = {
                "reagents": [{"reagent_id": "R456", "initial_quantity": {"value": 50, "unit": "uL"}, "current_quantity": {"value": 20, "unit": "uL"}}],
            ...
            assert updated_instance is mock_instance # Should return the same modified mock object

            # Check log message
            assert f"DEBUG: ADS: Updating properties_json for labware instance ID 1: {properties_update}" in caplog.text
