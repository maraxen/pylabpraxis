...
from praxis.backend.utils.state import State as PraxisState # Assuming State is aliased as PraxisState typically
...
class TestPraxisState:
    ...
    def test_init_requires_run_guid(self):
        with pytest.raises(ValueError, match="run_guid must be a non-empty string"):
            PraxisState(run_guid="") # type: ignore
        with pytest.raises(ValueError, match="run_guid must be a non-empty string"):
            PraxisState(run_guid="   ") # type: ignore

    def test_init_connects_to_redis_and_loads_initial_state(self, mock_redis_client: MagicMock):
        mock_redis_client.get.return_value = json.dumps({"initial_key": "initial_value"}).encode('utf-8')
        
        state = PraxisState(run_guid="test_run_123")
        ...
        assert state._data == {"initial_key": "initial_value"} # Check internal representation too

    def test_init_handles_empty_state_from_redis(self, mock_redis_client: MagicMock):
        mock_redis_client.get.return_value = None # No state in Redis for this run_guid
        
        state = PraxisState(run_guid="test_run_new")
    ...
    def test_init_handles_redis_connection_error(self, mock_redis_client: MagicMock):
        ...
        mock_redis_client.ping.side_effect = redis.exceptions.ConnectionError("Test connection error")
        
        with pytest.raises(ConnectionError, match="PraxisState: Failed to connect to Redis - Test connection error"):
            PraxisState(run_guid="test_run_conn_error")
    ...
    def test_init_handles_json_decode_error(self, mock_redis_client: MagicMock, caplog):
        ...
        state = PraxisState(run_guid="test_run_json_error")
    ...
    def test_setitem_saves_to_redis_and_updates_data(self, mock_redis_client: MagicMock):
        ...
        state = PraxisState(run_guid="test_run_set")
        ...
        mock_redis_client.set.assert_called_once_with(
            ...
            json.dumps({"my_key": "my_value"}).encode('utf-8')
        )

        state["another_key"] = {"nested": 123}
        mock_redis_client.set.assert_called_with(
            "praxis_state:test_run_set",
            json.dumps({"my_key": "my_value", "another_key": {"nested": 123}}).encode('utf-8')
        )
    ...
    def test_getitem_retrieves_from_data(self, mock_redis_client: MagicMock):
        ...
        state = PraxisState(run_guid="test_run_get")
    ...
    def test_delitem_saves_to_redis_and_updates_data(self, mock_redis_client: MagicMock):
        ...
        mock_redis_client.get.return_value = json.dumps(initial_data).encode('utf-8')
        state = PraxisState(run_guid="test_run_del")

        del state["key_to_del"]
        
        assert "key_to_del" not in state._data
        mock_redis_client.set.assert_called_once_with(
            "praxis_state:test_run_del",
            json.dumps({"key1": "val1"}).encode('utf-8')
        )

    def test_get_method(self, mock_redis_client: MagicMock):
        initial_data = {"key1": "val1"}
        mock_redis_client.get.return_value = json.dumps(initial_data).encode('utf-8')
        state = PraxisState(run_guid="test_run_get_method")
    ...
    def test_update_method_saves_to_redis(self, mock_redis_client: MagicMock):
        mock_redis_client.get.return_value = json.dumps({"key1": "val1_orig"}).encode('utf-8')
        state = PraxisState(run_guid="test_run_update")
    ...
    def test_to_dict_returns_copy(self, mock_redis_client: MagicMock):
        initial_data = {"key1": "val1"}
        mock_redis_client.get.return_value = json.dumps(initial_data).encode('utf-8')
        state = PraxisState(run_guid="test_run_to_dict")
    ...
    def test_clear_clears_data_and_deletes_from_redis(self, mock_redis_client: MagicMock):
        initial_data = {"key1": "val1"}
        mock_redis_client.get.return_value = json.dumps(initial_data).encode('utf-8')
        state = PraxisState(run_guid="test_run_clear")
    ...
    def test_attribute_access_set_and_get(self, mock_redis_client: MagicMock):
        ...
        state = PraxisState(run_guid="test_run_attr")
        ...
        mock_redis_client.set.assert_called_once_with(
            "praxis_state:test_run_attr",
            json.dumps(expected_data).encode('utf-8')
        )

    def test_attribute_access_get_non_existent(self, mock_redis_client: MagicMock):
        mock_redis_client.get.return_value = None
        state = PraxisState(run_guid="test_run_attr_get_fail")
    ...
    def test_set_internal_attribute(self, mock_redis_client: MagicMock):
        mock_redis_client.get.return_value = None
        state = PraxisState(run_guid="test_run_internal_attr")
        
        # These should not go into _data or trigger Redis save
        state.run_guid = "new_guid" # Should be handled by super().__setattr__
        state._data = {"internal_override": "danger"} # Should be handled by super().__setattr__
        
        assert state.run_guid == "new_guid"
