...
app = FastAPI()
app.include_router(protocols_router, prefix="/api/protocols") # Match the prefix used in tests

client = TestClient(app)

# A valid run_guid for testing
TEST_RUN_GUID = "test-run-guid-123"
...
class TestProtocolCommandsAPI:
    ...
    def test_send_pause_command_success(self, mock_send_control_command_redis):
        ...
        response = client.post(f"/api/protocols/{TEST_RUN_GUID}/command", params={"command": "PAUSE"})
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {"message": f"Command 'PAUSE' sent to run '{TEST_RUN_GUID}'"}
    ...
    def test_send_resume_command_success(self, mock_send_control_command_redis):
        mock_send_control_command_redis.return_value = True
        response = client.post(f"/api/protocols/{TEST_RUN_GUID}/command", params={"command": "RESUME"})
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {"message": f"Command 'RESUME' sent to run '{TEST_RUN_GUID}'"}
    ...
    def test_send_cancel_command_success(self, mock_send_control_command_redis):
        mock_send_control_command_redis.return_value = True
        response = client.post(f"/api/protocols/{TEST_RUN_GUID}/command", params={"command": "CANCEL"})
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {"message": f"Command 'CANCEL' sent to run '{TEST_RUN_GUID}'"}
    ...
    def test_send_invalid_command(self, mock_send_control_command_redis):
        ...
        response = client.post(f"/api/protocols/{TEST_RUN_GUID}/command", params={"command": invalid_command})
    ...
    def test_send_command_redis_error(self, mock_send_control_command_redis):
        mock_send_control_command_redis.side_effect = redis.exceptions.RedisError("Test Redis Error")
        response = client.post(f"/api/protocols/{TEST_RUN_GUID}/command", params={"command": "PAUSE"})
        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert "detail" in response.json()
        assert "An unexpected error occurred while sending command" in response.json()["detail"]
        mock_send_control_command_redis.assert_called_once_with(TEST_RUN_GUID, "PAUSE")

    @patch("praxis.backend.api.protocols.send_control_command_to_redis")
    def test_send_command_failure_from_redis_false(self, mock_send_control_command_redis):
        mock_send_control_command_redis.return_value = False # Simulate Redis command sending failure
        response = client.post(f"/api/protocols/{TEST_RUN_GUID}/command", params={"command": "PAUSE"})
        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert "detail" in response.json()
        assert f"Failed to send command to run '{TEST_RUN_GUID}' using Redis." in response.json()["detail"]
        mock_send_control_command_redis.assert_called_once_with(TEST_RUN_GUID, "PAUSE")
