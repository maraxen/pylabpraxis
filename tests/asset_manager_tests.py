...
class TestAssetManagerLogging:
    ...
    def test_acquire_labware_logs_property_constraints(self, asset_manager: AssetManager, mock_ads_service: MagicMock, mock_workcell_runtime: MagicMock, caplog):
        # Setup for a successful labware acquisition to reach the logging point
        mock_lw_instance_orm = LabwareInstanceOrmMock(id=1, user_assigned_name="PlateLogTest", pylabrobot_definition_name="log_plate_def")
        mock_labware_def = LabwareDefinitionCatalogOrmMock()
        mock_labware_def.python_fqn = "pylabrobot.resources.plate.Plate" # Valid FQN
        
        mock_ads_service.list_labware_instances.return_value = [mock_lw_instance_orm]
        mock_ads_service.get_labware_definition.return_value = mock_labware_def
        mock_ads_service.update_labware_instance_location_and_status.return_value = mock_lw_instance_orm
        mock_workcell_runtime.create_or_get_labware_plr_object.return_value = MagicMock(name="live_logging_plate")
        ...
        asset_req.actual_type_str = "log_plate_def" # Matches pylabrobot_definition_name_constraint
        asset_req.constraints_json = property_constraints_payload # This is what we want to log

        # Call acquire_asset, which dispatches to acquire_labware
        try:
        ...
        assert f"INFO: AM_ACQUIRE: Labware acquisition for 'test_plate_for_log' includes property_constraints: {property_constraints_payload}" in caplog.text

    def test_release_labware_logs_properties_update(self, asset_manager: AssetManager, mock_ads_service: MagicMock, caplog):
        # Setup for release_labware
        final_props_update = {"content_state": "empty", "cleaned": True}
        
        # Mock the get_labware_instance_by_id call that might happen if workcell_runtime is None or other logic paths
        # For this test, we are primarily interested in the log message generated by release_labware itself.
        # The actual update logic is tested elsewhere.
        mock_ads_service.update_labware_instance_location_and_status.return_value = MagicMock(spec=LabwareInstanceOrm)


        asset_manager.release_labware(
            labware_instance_orm_id=555,
            final_status=LabwareInstanceStatusEnum.AVAILABLE_IN_STORAGE,
            final_properties_json_update=final_props_update,
            status_details="Logging properties update on release"
        )
        
        assert f"INFO: AM_RELEASE: Labware release for instance ID 555 includes final_properties_json_update: {final_props_update}" in caplog.text
```
