...
from praxis.protocol_core.definitions import Deck as ProtocolDeck
...
class TestAssetManagerAcquireDevice:
  def test_acquire_machine_success(
    self,
  ...
  def test_acquire_machine_no_machine_found(
    ...
    with pytest.raises(
      ...
      match="No machine found matching type constraint 'SomeDeviceClass' and status AVAILABLE.",
    ):
      asset_manager.acquire_machine("run123", "dev", "SomeDeviceClass")
      # Added to resolve SyntaxError

  def test_acquire_machine_backend_init_fails(
    self,
    asset_manager: AssetManager,
    mock_ads_service: MagicMock,
    mock_workcell_runtime: MagicMock,
  ...
  def test_acquire_machine_db_status_update_fails_after_init(
    ...
    with pytest.raises(
      AssetAcquisitionError,
      match="CRITICAL: Device 'Device1' backend is live, but FAILED to update its DB status to IN_USE",
    ):
      asset_manager.acquire_machine("run123", "dev", "SomeDeviceClass")
...
class TestAssetManagerAcquireResource:
  ...
  def test_acquire_resource_success_from_storage(
    ...
    mock_ads_service.update_resource_location_and_status.assert_called_with(
      asset_manager.db,
      mock_lw_orm.accession_id,
  ...
  def test_acquire_resource_fqn_not_found(
    ...
    with pytest.raises(
      AssetAcquisitionError,
      match="Python FQN not found for resource definition 'unknown_def_name'",
    ):
      pass  # Added to resolve SyntaxError

    mock_ads_service.update_resource_location_and_status.assert_called_once_with(
      db=asset_manager.db,
      resource_accession_id=mock_lw_orm.accession_id,
  ...
  def test_acquire_resource_plr_object_creation_fails(
    ...
    mock_lw_orm = ResourceOrmMock(id=1, name="Plate1", resource_definition_name="def")
    mock_ads_service.read_resources.return_value = [mock_lw_orm]
    mock_ads_service.get_resource_definition.return_value = mock_resource_def
...
class TestAssetManagerRelease:
  ...
  def test_release_resource_with_deck_clear(
    ...
    mock_workcell_runtime.clear_deck_slot.assert_called_once_with(
      deck_machine_orm_accession_id=10,
      slot_name="A1",
      resource_orm_accession_id=1,
    )
    mock_ads_service.update_resource_location_and_status.assert_called_once_with(
...
class TestAssetManagerDeckLoading:
  ...
  def mock_slot_orm(self, mock_resource_orm):
    ...
    slot.slot_name = "A1"
    slot.pre_assigned_resource_accession_id = mock_resource_orm.accession_id
  ...
  def mock_resource_def_catalog_orm(self):
    lw_def = MagicMock(spec=ResourceDefinitionOrm)
    lw_def.name = "test_plate_def_name"
    lw_def.fqn = "pylabrobot.resources.plate.Plate"
    return lw_def

  def test_deck_layout_not_found(self, asset_manager: AssetManager, mock_ads_service: MagicMock) -> None:
  ...
  def test_deck_machine_init_fails(
    ...
    asset_manager: AssetManager,
    mock_ads_service: MagicMock,
    mock_deck_layout_orm,
    mock_deck_machine_orm,
    mock_workcell_runtime: MagicMock,
  ) -> None:
    mock_ads_service.get_deck_layout_by_name.return_value = mock_deck_layout_orm
  ...
  def test_successful_deck_config_no_resource(
    ...
    returned_deck = asset_manager.apply_deck("TestDeckLayout", "run123")

    assert returned_deck == live_plr_deck_obj
    mock_ads_service.update_managed_machine_status.assert_called_once_with(
      asset_manager.db,
      mock_deck_machine_orm.accession_id,
      ManagedDeviceStatusEnum.IN_USE,
      current_protocol_run_accession_id="run123",
  ...
  def test_successful_deck_config_with_one_resource(
    ...
    mock_ads_service.get_resource.return_value = mock_resource_orm
    ...
    mock_ads_service.update_resource_location_and_status.assert_called_once_with(
      db=asset_manager.db,
      resource_accession_id=mock_resource_orm.accession_id,
      new_status=ResourceStatusEnum.IN_USE,
      current_protocol_run_accession_id="run123",
  ...
  def test_resource_not_available_for_slot(
    self,
    asset_manager: AssetManager,
    mock_ads_service: MagicMock,
    mock_deck_layout_orm,
  ...
  def test_resource_def_fqn_not_found_for_slot_resource(
    ...
    mock_resource_def_catalog_orm.fqn = None  # FQN missing
