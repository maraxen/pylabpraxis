...
@pytest.mark.asyncio
async def test_schedule_protocol_execution(service, scheduler, mock_protocol_definition_service, mock_protocol_run_service) -> None:
    """Test scheduling a protocol for asynchronous execution."""
    protocol_def_orm = MagicMock()
    protocol_def_orm.accession_id = uuid.uuid4()
    mock_protocol_definition_service.get_by_name_and_version = AsyncMock(return_value=protocol_def_orm)
    mock_protocol_run_service.create = AsyncMock(return_value=MagicMock())
    ...
    mock_protocol_run_service.create.assert_awaited_once()


@pytest.mark.asyncio
async def test_get_protocol_run_status(service, scheduler, mock_protocol_run_service) -> None:
    ...
    mock_protocol_run_service.get.return_value = protocol_run_orm
    db_session = MagicMock()
    service.db_session_factory.return_value.__aenter__.return_value = db_session
...
@pytest.mark.asyncio
async def test_cancel_protocol_run(service, scheduler, mock_protocol_run_service) -> None:
    """Test cancelling a protocol run."""
    protocol_run_id = uuid.uuid4()
    mock_protocol_run_service.update_run_status = AsyncMock()
    db_session = MagicMock()
    db_session.commit = AsyncMock()
    service.db_session_factory.return_value.__aenter__.return_value = db_session
    ...
    mock_protocol_run_service.update_run_status.assert_awaited_once()
