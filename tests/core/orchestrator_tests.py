...
@pytest.fixture
def mock_protocol_run_service() -> MagicMock:
    """Provide a mock ProtocolRunService."""
...
def mock_protocol_definition_service() -> MagicMock:
    """Provide a mock ProtocolDefinitionCRUDService."""
    service = MagicMock(spec=ProtocolDefinitionCRUDService)
    service.get_by_name_and_version = AsyncMock()
    return service
...
def orchestrator(
    ...
    mock_protocol_run_service: MagicMock,
...
class TestOrchestratorExecution:
    ...
    async def test_execute_protocol_happy_path(
        ...
        orchestrator.protocol_definition_service.get_by_name_and_version.return_value = (
            mock_protocol_definition
        )
        mock_asset_manager = orchestrator.asset_manager
        mock_asset_manager.acquire_asset.return_value = (
            MagicMock(),
            uuid.uuid4(),
            "resource",
        )
        mock_protocol_func = AsyncMock(return_value={"result": "success"})
        orchestrator.protocol_code_manager.prepare_protocol_code.return_value = (
            mock_protocol_func,
            mock_protocol_definition,
        )
        ...
        orchestrator.protocol_run_service.create.assert_awaited_once()
        assert orchestrator.protocol_run_service.update.call_count >= 1
    ...
    async def test_execute_protocol_handles_exception(
        ...
        assert orchestrator.protocol_run_service.update.call_count >= 1
        final_call = orchestrator.protocol_run_service.update.call_args_list[-1]
...
class TestOrchestratorScenarios:
    ...
    async def test_execute_raises_error_if_protocol_not_found(
        ...
        orchestrator.protocol_definition_service.get_by_name_and_version.return_value = None
    ...
    async def test_execute_raises_error_if_asset_is_missing(
        ...
        orchestrator.asset_manager.acquire_asset.side_effect = ValueError(
            "Asset not found"
        )
