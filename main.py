...
from praxis.backend.services.praxis_orm_service import (
  PraxisDBService,
)  # Assuming praxis_orm_service.py is in praxis/
...
)  # TODO: refactor Orchestrator to accept db_service_instance
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:  # Corrected type hint
  ...
  try:
    ...
    logger.info(
      f"Default protocol directory from config: {praxis_config.default_protocol_dir}"
    )
    ...
    try:
      ...
      if (
        hasattr(orchestrator, "db_service") and orchestrator.db_service
      ):  # Check if orchestrator has the service
...
async def log_requests(
  request: Request, call_next: Awaitable[Response]
) -> Response:  # Corrected Awaitable type
