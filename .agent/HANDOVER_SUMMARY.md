...
These are the files you should use to guide the next steps. They are located in `.agent/artifacts/`:

1. **[Workcell UX Redesign](.agent/artifacts/workcell_ux_redesign.md)**:
...
2. **[Unified Asset Selector Design](.agent/artifacts/inventory_ux_design.md)**:
...
3. **[Simulation Architecture Plan](.agent/artifacts/simulation_architecture_plan.md)**:
...
If you need to revisit the rationale for recent changes, see `.agent/archive/archive.tar.gz`. Specifically, look at `summaries/pylabpraxis_batch_260114.md` for a mapping of all recently completed tasks.
