"""plr_jit._hand_maintained: the hand-maintained surface registry (spec
260901 §9, task 260901_plr-jit-t9-registry-ratchet, backlog #4835).

**A data module, not an analysis module.** Every hand-typed fact `plr_jit`
...
reviewable commit). Run it with:
    ...
    uv run python -m plr_jit._hand_maintained --update-baselines
"""
...
# lives under `src/plr_jit/`, and `tests/test_import_boundary.py` AST-scans
...
# included (spec §1.3 AC-1.2 / AC-1.3 -- `plr_jit` must stay corpus- and
...
def _measure_hm14() -> int:
    from plr_jit.verdict import REASON_VOCABULARY
...
def _measure_hm16() -> int:
    ...
    (spec §1.2): `from plr_jit.<mod> import *` re-export shims. Round 1 has
    ...
    for path in sorted(shim_dir.glob("*.py")):
        if path.name == "__init__.py":
            continue
        text = path.read_text()
        if "from plr_jit." in text and "import *" in text:
            count += 1
    return count
...
REGISTRY: tuple[HandMaintainedSurface, ...] = (
    ...
    HandMaintainedSurface(
        ...
        breaks_when=(
            ...
            "plr_jit.telemetry and training/verify/failure_taxonomy.py."
        ),
    ...
    HandMaintainedSurface(
        ...
        measure="plr_jit._hand_maintained:_measure_hm8",
    ),
    HandMaintainedSurface(
        id="HM-9",
        what="SUPPORTED_TOOLS (plr_jit/check/_supported_tools.py; mirrors training/verify/dispatcher.py)",
        metric="tools",
        declared=10,
        status="CAPPED",
        why_not_derived=(
            ...
            "training/verify/, orthogonal to what plr_jit derives or "
        ...
        measure="plr_jit._hand_maintained:_measure_hm9",
    ),
    HandMaintainedSurface(
        id="HM-10",
        ...
        measure="plr_jit._hand_maintained:_measure_hm10",
    ),
    HandMaintainedSurface(
        id="HM-11",
    ...
    HandMaintainedSurface(
        ...
        measure="plr_jit._hand_maintained:_measure_hm12",
    ),
    HandMaintainedSurface(
        id="HM-13",
        ...
        measure="plr_jit._hand_maintained:_measure_hm13",
    ),
    HandMaintainedSurface(
        id="HM-14",
        what="REASON_VOCABULARY (plr_jit/verdict.py, section 3.3)",
        metric="reasons",
        declared=12,
        status="CAPPED",
        why_not_derived=(
        ...
        measure="plr_jit._hand_maintained:_measure_hm14",
    ),
    HandMaintainedSurface(
        id="HM-15",
    ...
    HandMaintainedSurface(
        ...
        status="CAPPED",
        why_not_derived=(
            "A byproduct of the praxis -> plr_jit migration itself; the "
        ...
        measure="plr_jit._hand_maintained:_measure_hm16",
    ),
    HandMaintainedSurface(
        id="HM-17",
        what="picked git_state.py (plr_jit/_provenance/git_state.py, section 2)",
        ...
        measure="plr_jit._hand_maintained:_measure_hm17",
    ),
    HandMaintainedSurface(
        id="HM-18",
        ...
        measure="plr_jit._hand_maintained:_measure_hm18",
    ),
    HandMaintainedSurface(
        id="HM-19",
    ...
    HandMaintainedSurface(
        ...
        measure="plr_jit._hand_maintained:_measure_hm21",
    ),
    HandMaintainedSurface(
        id="HM-22",
        what=(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="plr_jit._hand_maintained")
    parser.add_argument(
        "--update-baselines",
        action="store_true",
