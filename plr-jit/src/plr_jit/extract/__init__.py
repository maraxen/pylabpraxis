"""plr_jit.extract: the server-side extractor (spec 260901 §6.2).

**Round 2, not round 1.** ``src/plr_jit/extract/`` is where ``libcst`` and
``pylabrobot`` imports are permitted (§6.1/§6.2) -- source-in,
``ProtocolComputationGraph``-JSON-out, feeding ``plr_jit.check`` over the
