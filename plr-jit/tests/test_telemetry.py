...
import-boundary walk scopes to src/plr_jit/ only -- importing verify from a
test here is legal. Importing verify from src/plr_jit/telemetry.py would
...
from plr_jit.verdict import Finding, PlrSite, Verdict
from plr_jit import telemetry
from plr_jit.telemetry import (
    FAILURE_CATEGORIES,
    JsonlSink,
    build_event,
    emit,
...
# AC-4.3) -- not a fact about plr-jit itself, so if the submodule is ever
...
def test_categories_match_upstream() -> None:
    ...
    assert FAILURE_CATEGORIES == upstream_taxonomy.FAILURE_CATEGORIES, (
        f"plr_jit.telemetry.FAILURE_CATEGORIES "
...
def test_event_carries_stamp(
    ...
    if case == "real_repo":
        ...
        from plr_jit._provenance.git_state import _UNAVAILABLE
