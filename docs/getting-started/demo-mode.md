...
Praxis supports a frontend-only demo mode that runs entirely in the browser without requiring a backend server.
...
For persistence across page reloads, demo mode uses SQL.js (SQLite compiled to WebAssembly):
...
When running protocols in demo mode:
...
!!! warning "Demo Mode Security"
    ...
    2. **Separate build config**: Uses `environment.demo.ts`
