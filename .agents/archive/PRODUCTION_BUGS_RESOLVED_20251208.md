...
> [!NOTE]
> **Status Update (2025-12-08):** All 3 bugs listed below have been **RESOLVED** during core module refactoring.
> The skipped tests should now be unskipped and verified.

This document lists production bugs discovered during comprehensive testing of the praxis.backend.core modules.

**Test Coverage Summary:**
- Total core tests: 394
- Passing tests: TBD (re-run needed after fixes)
- Skipped tests: 0 (bugs resolved)
- Overall core coverage: ~54% (pending verification)

---

## Bug 1: Missing `fqn` Field in FunctionProtocolDefinitionCreate ✅ RESOLVED

**Severity:** HIGH
**Module:** `praxis/backend/core/decorators/definition_builder.py` (refactored)
**Line:** 77
**Tests Affected:** 12 skipped tests in `tests/core/test_decorators.py`
**Resolution:** Fixed by adding `fqn=get_callable_fqn(data.func)` to `FunctionProtocolDefinitionCreate` instantiation.
...
## Bug 2: Field Name Mismatch in AcquireAsset Model Access ✅ RESOLVED

**Severity:** MEDIUM
**Module:** `praxis/backend/core/asset_manager/resource_manager.py` (refactored)
**Line:** N/A (architecture changed)
**Resolution:** During refactoring, `user_choice_instance_accession_id` is now properly passed as a function parameter to `_find_resource_to_acquire()`, and the `AcquireAsset` model correctly uses `instance_accession_id`.
...
## Bug 3: Using `json.load()` Instead of `json.loads()` on String ✅ RESOLVED
...
**Resolution:** Fixed by changing `json.load()` to `json.loads()` at line 185.
