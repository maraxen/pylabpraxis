...
**Created**: 2026-01-05
**Status**: ✅ Complete (audited 2026-01-06)
...
### Phase 1: Add Simulation Indicator ✅ COMPLETE

- [x] Add `is_simulated` field to Machine model (implemented via `is_simulation_override` column)
- [x] Create "Simulated" chip component (`SimulatedChipComponent`)
- [x] Display chip in machine cards, selection step, deck setup
- [x] Unit tests (MachineCard has `isSimulated()` method)

### Phase 2: Default Asset Seeding ✅ COMPLETE

- [x] Refactor `seedMockData()` to `seedDefaultAssets()` (in `sqlite.service.ts`)
- [x] Implement 1-of-each-type logic for resources
- [x] Implement 1-of-each-type with sim backend for machines (`is_simulation_override=1`)
- [x] Seeds only when database has no existing assets

### Phase 3: Infinite Consumables ✅ COMPLETE

- [x] `infiniteConsumables` setting in `AppStore` (default: true)
- [x] Toggle in Settings component
- [x] Infinite symbol display in resource accordion (`isInfinite` computed property)
- [x] LocalStorage persistence

### Phase 4: Demo Mode Removal ✅ COMPLETE

- [x] `DemoInterceptor` renamed conceptually (still exists but serves Browser Mode API mocking)
- [x] `WelcomeDialogComponent` - no demo toggle, only tutorial start
- [x] `SettingsComponent` - no demo toggle
- [x] `ExitDemoDialogComponent` never existed
- [x] `ModeService` - no `isDemoMode` method
- [x] `OnboardingService` - no demo state tracking

### Phase 5: Reset to Defaults ✅ COMPLETE

- [x] `resetToDefaults()` function implemented in `SqliteService`
- [x] `ConfirmationDialogComponent` for confirmation
- [x] "Reset to Defaults" button in Settings component
- [x] IndexedDB persistence after reset
...
1. [x] Browser mode starts with populated inventory (no empty state for new users)
2. [x] All seeded machines show "Simulated" indicator
3. [x] Infinite consumables toggle exists (auto-replenish at execution time is not implemented - UI indicator only)
4. [x] No demo mode toggle anywhere in UI
5. [x] "Reset to Defaults" works correctly
6. [x] Welcome dialog shows Browser Mode note
