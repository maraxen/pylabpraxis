...
- [ ] **Maintenance Audit Prompt**: Create a generic reusable prompt that conducts audits to see what maintenance or other reusable prompts should be used, or if the `.agents/archive` needs to be decluttered. <!-- id: 501 -->
  ...
  - Priority: Low
  - Notes: This prompt should analyze the state of `.agents/prompts`, `.agents/archive`, and `.agents/artifacts` to suggest cleanup or improvements.
- [ ] **Structured Agentic Multi-Stage Workflow**: Implement a new skill or reusable prompt that formalizes the end-to-end development cycle. <!-- id: 502 -->
  ...
  - Notes: The workflow should follow this strict process:
    ...
    2. **Specification & Inspection**: Once the issue is fully specified, use inspection prompts to generate discovery artifacts (output to `.agents/artifacts`). Update backlog and `DEVELOPMENT_MATRIX.md` as appropriate.
    ...
    - **Action Item**: Create an artifact template (`.agents/templates/artifact.md`) as we currently lack one.
