...
import { AvailableValuesSection } from './values/AvailableValuesSection';
import { MetadataManager } from '@protocols/managers/MetadataManager';
import { EditingManager } from '@protocols/managers/editingManager';
import { DebugManager } from '@protocols/managers/debugManager';
import { ValueDisplay } from './values/ValueDisplay';
...
const HierarchicalMappingImpl: React.FC<NestedMappingProps> = ({
  ...
  // Get the value type from constraints
  ...
  useEffect(() => {
    ...
    Object.entries(updatedValue).forEach(([key, groupData]) => {
      ...
      const group = groupData as Partial<GroupData>;
  ...
  const handleDragEnd = (event: any) => {
    ...
    try {
      ...
      if (targetId === 'available-values' && sourceGroupId) {
        const sourceGroup = value[sourceGroupId] as GroupData;
      ...
      // Handle dropping to a group
        ...
        const targetGroup = value[targetId] as GroupData;
